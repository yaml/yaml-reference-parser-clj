;; This grammar was generated from https://yaml.org/spec/1.2/spec.html

(ns yaml-parser.grammar
  (:refer-clojure :exclude [empty])
  (:require [yaml-parser.parser :as p]
            [yaml-parser.prelude
             :refer [debug-rule name* from-code-point DEBUG]]))

(declare l_yaml_stream)

;; TOP returns the top-level grammar rule as a vector for call to process
(def TOP
  (name* "TOP"
    (fn TOP-fn [parser]
      ;; Return the top rule as a value that call can process
      [l_yaml_stream])
    nil))

;; Helper function for auto-detect
(defn auto_detect [parser n]
  (p/auto-detect parser n))

(defn auto_detect_indent [parser n]
  (p/auto-detect-indent parser n))

(defn empty [parser]
  (p/empty-rule parser))

(declare
  c_printable
  nb_json
  c_byte_order_mark
  c_sequence_entry
  c_mapping_key
  c_mapping_value
  c_collect_entry
  c_sequence_start
  c_sequence_end
  c_mapping_start
  c_mapping_end
  c_comment
  c_anchor
  c_alias
  c_tag
  c_literal
  c_folded
  c_single_quote
  c_double_quote
  c_directive
  c_reserved
  c_indicator
  c_flow_indicator
  b_line_feed
  b_carriage_return
  b_char
  nb_char
  b_break
  b_as_line_feed
  b_non_content
  s_space
  s_tab
  s_white
  ns_char
  ns_dec_digit
  ns_hex_digit
  ns_ascii_letter
  ns_word_char
  ns_uri_char
  ns_tag_char
  c_escape
  ns_esc_null
  ns_esc_bell
  ns_esc_backspace
  ns_esc_horizontal_tab
  ns_esc_line_feed
  ns_esc_vertical_tab
  ns_esc_form_feed
  ns_esc_carriage_return
  ns_esc_escape
  ns_esc_space
  ns_esc_double_quote
  ns_esc_slash
  ns_esc_backslash
  ns_esc_next_line
  ns_esc_non_breaking_space
  ns_esc_line_separator
  ns_esc_paragraph_separator
  ns_esc_8_bit
  ns_esc_16_bit
  ns_esc_32_bit
  c_ns_esc_char
  s_indent
  s_indent_lt
  s_indent_le
  s_separate_in_line
  s_line_prefix
  s_block_line_prefix
  s_flow_line_prefix
  l_empty
  b_l_trimmed
  b_as_space
  b_l_folded
  s_flow_folded
  c_nb_comment_text
  b_comment
  s_b_comment
  l_comment
  s_l_comments
  s_separate
  s_separate_lines
  l_directive
  ns_reserved_directive
  ns_directive_name
  ns_directive_parameter
  ns_yaml_directive
  ns_yaml_version
  ns_tag_directive
  c_tag_handle
  c_primary_tag_handle
  c_secondary_tag_handle
  c_named_tag_handle
  ns_tag_prefix
  c_ns_local_tag_prefix
  ns_global_tag_prefix
  c_ns_properties
  c_ns_tag_property
  c_verbatim_tag
  c_ns_shorthand_tag
  c_non_specific_tag
  c_ns_anchor_property
  ns_anchor_char
  ns_anchor_name
  c_ns_alias_node
  e_scalar
  e_node
  nb_double_char
  ns_double_char
  c_double_quoted
  nb_double_text
  nb_double_one_line
  s_double_escaped
  s_double_break
  nb_ns_double_in_line
  s_double_next_line
  nb_double_multi_line
  c_quoted_quote
  nb_single_char
  ns_single_char
  c_single_quoted
  nb_single_text
  nb_single_one_line
  nb_ns_single_in_line
  s_single_next_line
  nb_single_multi_line
  ns_plain_first
  ns_plain_safe
  ns_plain_safe_out
  ns_plain_safe_in
  ns_plain_char
  ns_plain
  nb_ns_plain_in_line
  ns_plain_one_line
  s_ns_plain_next_line
  ns_plain_multi_line
  in_flow
  c_flow_sequence
  ns_s_flow_seq_entries
  ns_flow_seq_entry
  c_flow_mapping
  ns_s_flow_map_entries
  ns_flow_map_entry
  ns_flow_map_explicit_entry
  ns_flow_map_implicit_entry
  ns_flow_map_yaml_key_entry
  c_ns_flow_map_empty_key_entry
  c_ns_flow_map_separate_value
  c_ns_flow_map_json_key_entry
  c_ns_flow_map_adjacent_value
  ns_flow_pair
  ns_flow_pair_entry
  ns_flow_pair_yaml_key_entry
  c_ns_flow_pair_json_key_entry
  ns_s_implicit_yaml_key
  c_s_implicit_json_key
  ns_flow_yaml_content
  c_flow_json_content
  ns_flow_content
  ns_flow_yaml_node
  c_flow_json_node
  ns_flow_node
  c_b_block_header
  c_indentation_indicator
  c_chomping_indicator
  b_chomped_last
  l_chomped_empty
  l_strip_empty
  l_keep_empty
  l_trail_comments
  c_l_literal
  l_nb_literal_text
  b_nb_literal_next
  l_literal_content
  c_l_folded
  s_nb_folded_text
  l_nb_folded_lines
  s_nb_spaced_text
  b_l_spaced
  l_nb_spaced_lines
  l_nb_same_lines
  l_nb_diff_lines
  l_folded_content
  l_block_sequence
  c_l_block_seq_entry
  s_l_block_indented
  ns_l_compact_sequence
  l_block_mapping
  ns_l_block_map_entry
  c_l_block_map_explicit_entry
  c_l_block_map_explicit_key
  l_block_map_explicit_value
  ns_l_block_map_implicit_entry
  ns_s_block_map_implicit_key
  c_l_block_map_implicit_value
  ns_l_compact_mapping
  s_l_block_node
  s_l_flow_in_block
  s_l_block_in_block
  s_l_block_scalar
  s_l_block_collection
  seq_spaces
  l_document_prefix
  c_directives_end
  c_document_end
  l_document_suffix
  c_forbidden
  l_bare_document
  l_explicit_document
  l_directive_document
  l_any_document
  l_yaml_stream)

;; [001]
;; c-printable ::=
;;   x:9 | x:A | x:D | [x:20-x:7E]
;;   | x:85 | [x:A0-x:D7FF] | [x:E000-x:FFFD]
;;   | [x:10000-x:10FFFF]
(def c_printable
  (let [body (p/chars nil [0x9 0xA 0xD 0xD 0x20 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFFFD 0x10000 0x10FFFF])]
    (p/leaf*
     (name* "c_printable"
       (fn c_printable-fn [parser]
         (when DEBUG (debug-rule "c_printable"))
         (body parser))
       nil))))
;; [002]
;; nb-json ::=
;;   x:9 | [x:20-x:10FFFF]
(def nb_json
  (let [body (p/chars nil [0x9 0x9 0x20 0x10FFFF])]
    (p/leaf*
     (name* "nb_json"
       (fn nb_json-fn [parser]
         (when DEBUG (debug-rule "nb_json"))
         (body parser))
       nil))))
;; [003]
;; c-byte-order-mark ::=
;;   x:FEFF
(def c_byte_order_mark
  (let [body (p/chr nil "\uFEFF")]
    (p/leaf*
     (name* "c_byte_order_mark"
       (fn c_byte_order_mark-fn [parser]
         (when DEBUG (debug-rule "c_byte_order_mark"))
         (body parser))
       nil))))
;; [004]
;; c-sequence-entry ::=
;;   '-'
(def c_sequence_entry
  (let [body (p/chr nil "\u002D")]
    (p/leaf*
     (name* "c_sequence_entry"
       (fn c_sequence_entry-fn [parser]
         (when DEBUG (debug-rule "c_sequence_entry"))
         (body parser))
       nil))))
;; [005]
;; c-mapping-key ::=
;;   '?'
(def c_mapping_key
  (let [body (p/chr nil "\u003F")]
    (p/leaf*
     (name* "c_mapping_key"
       (fn c_mapping_key-fn [parser]
         (when DEBUG (debug-rule "c_mapping_key"))
         (body parser))
       nil))))
;; [006]
;; c-mapping-value ::=
;;   ':'
(def c_mapping_value
  (let [body (p/chr nil "\u003A")]
    (p/leaf*
     (name* "c_mapping_value"
       (fn c_mapping_value-fn [parser]
         (when DEBUG (debug-rule "c_mapping_value"))
         (body parser))
       nil))))
;; [007]
;; c-collect-entry ::=
;;   ','
(def c_collect_entry
  (let [body (p/chr nil "\u002C")]
    (p/leaf*
     (name* "c_collect_entry"
       (fn c_collect_entry-fn [parser]
         (when DEBUG (debug-rule "c_collect_entry"))
         (body parser))
       nil))))
;; [008]
;; c-sequence-start ::=
;;   '['
(def c_sequence_start
  (let [body (p/chr nil "\u005B")]
    (p/leaf*
     (name* "c_sequence_start"
       (fn c_sequence_start-fn [parser]
         (when DEBUG (debug-rule "c_sequence_start"))
         (body parser))
       nil))))
;; [009]
;; c-sequence-end ::=
;;   ']'
(def c_sequence_end
  (let [body (p/chr nil "\u005D")]
    (p/leaf*
     (name* "c_sequence_end"
       (fn c_sequence_end-fn [parser]
         (when DEBUG (debug-rule "c_sequence_end"))
         (body parser))
       nil))))
;; [010]
;; c-mapping-start ::=
;;   '{'
(def c_mapping_start
  (let [body (p/chr nil "\u007B")]
    (p/leaf*
     (name* "c_mapping_start"
       (fn c_mapping_start-fn [parser]
         (when DEBUG (debug-rule "c_mapping_start"))
         (body parser))
       nil))))
;; [011]
;; c-mapping-end ::=
;;   '}'
(def c_mapping_end
  (let [body (p/chr nil "\u007D")]
    (p/leaf*
     (name* "c_mapping_end"
       (fn c_mapping_end-fn [parser]
         (when DEBUG (debug-rule "c_mapping_end"))
         (body parser))
       nil))))
;; [012]
;; c-comment ::=
;;   '#'
(def c_comment
  (let [body (p/chr nil "\u0023")]
    (p/leaf*
     (name* "c_comment"
       (fn c_comment-fn [parser]
         (when DEBUG (debug-rule "c_comment"))
         (body parser))
       nil))))
;; [013]
;; c-anchor ::=
;;   '&'
(def c_anchor
  (let [body (p/chr nil "\u0026")]
    (p/leaf*
     (name* "c_anchor"
       (fn c_anchor-fn [parser]
         (when DEBUG (debug-rule "c_anchor"))
         (body parser))
       nil))))
;; [014]
;; c-alias ::=
;;   '*'
(def c_alias
  (let [body (p/chr nil "\u002A")]
    (p/leaf*
     (name* "c_alias"
       (fn c_alias-fn [parser]
         (when DEBUG (debug-rule "c_alias"))
         (body parser))
       nil))))
;; [015]
;; c-tag ::=
;;   '!'
(def c_tag
  (let [body (p/chr nil "\u0021")]
    (p/leaf*
     (name* "c_tag"
       (fn c_tag-fn [parser]
         (when DEBUG (debug-rule "c_tag"))
         (body parser))
       nil))))
;; [016]
;; c-literal ::=
;;   '|'
(def c_literal
  (let [body (p/chr nil "\u007C")]
    (p/leaf*
     (name* "c_literal"
       (fn c_literal-fn [parser]
         (when DEBUG (debug-rule "c_literal"))
         (body parser))
       nil))))
;; [017]
;; c-folded ::=
;;   '>'
(def c_folded
  (let [body (p/chr nil "\u003E")]
    (p/leaf*
     (name* "c_folded"
       (fn c_folded-fn [parser]
         (when DEBUG (debug-rule "c_folded"))
         (body parser))
       nil))))
;; [018]
;; c-single-quote ::=
;;   '''
(def c_single_quote
  (let [body (p/chr nil "\u0027")]
    (p/leaf*
     (name* "c_single_quote"
       (fn c_single_quote-fn [parser]
         (when DEBUG (debug-rule "c_single_quote"))
         (body parser))
       nil))))
;; [019]
;; c-double-quote ::=
;;   '"'
(def c_double_quote
  (let [body (p/chr nil "\u0022")]
    (p/leaf*
     (name* "c_double_quote"
       (fn c_double_quote-fn [parser]
         (when DEBUG (debug-rule "c_double_quote"))
         (body parser))
       nil))))
;; [020]
;; c-directive ::=
;;   '%'
(def c_directive
  (let [body (p/chr nil "\u0025")]
    (p/leaf*
     (name* "c_directive"
       (fn c_directive-fn [parser]
         (when DEBUG (debug-rule "c_directive"))
         (body parser))
       nil))))
;; [021]
;; c-reserved ::=
;;   '@' | '`'
(def c_reserved
  (let [body (p/chars nil [0x40 0x40 0x60 0x60])]
    (p/leaf*
     (name* "c_reserved"
       (fn c_reserved-fn [parser]
         (when DEBUG (debug-rule "c_reserved"))
         (body parser))
       nil))))
;; [022]
;; c-indicator ::=
;;   '-' | '?' | ':' | ',' | '[' | ']' | '{' | '}'
;;   | '#' | '&' | '*' | '!' | '|' | '>' | ''' | '"'
;;   | '%' | '@' | '`'
(def c_indicator
  (let [body (p/chars nil [0x21 0x23 0x25 0x27 0x2A 0x2A 0x2C 0x2D 0x3A 0x3A 0x3E 0x40 0x5B 0x5B 0x5D 0x5D 0x60 0x60 0x7B 0x7D])]
    (p/leaf*
     (name* "c_indicator"
       (fn c_indicator-fn [parser]
         (when DEBUG (debug-rule "c_indicator"))
         (body parser))
       nil))))
;; [023]
;; c-flow-indicator ::=
;;   ',' | '[' | ']' | '{' | '}'
(def c_flow_indicator
  (let [body (p/chars nil [0x2C 0x2C 0x5B 0x5B 0x5D 0x5D 0x7B 0x7B 0x7D 0x7D])]
    (p/leaf*
     (name* "c_flow_indicator"
       (fn c_flow_indicator-fn [parser]
         (when DEBUG (debug-rule "c_flow_indicator"))
         (body parser))
       nil))))
;; [024]
;; b-line-feed ::=
;;   x:A
(def b_line_feed
  (let [body (p/chr nil "\u000A")]
    (p/leaf*
     (name* "b_line_feed"
       (fn b_line_feed-fn [parser]
         (when DEBUG (debug-rule "b_line_feed"))
         (body parser))
       nil))))
;; [025]
;; b-carriage-return ::=
;;   x:D
(def b_carriage_return
  (let [body (p/chr nil "\u000D")]
    (p/leaf*
     (name* "b_carriage_return"
       (fn b_carriage_return-fn [parser]
         (when DEBUG (debug-rule "b_carriage_return"))
         (body parser))
       nil))))
;; [026]
;; b-char ::=
;;   b-line-feed | b-carriage-return
(def b_char
  (let [body (p/chars nil [0xA 0xA 0xD 0xD])]
    (p/leaf*
     (name* "b_char"
       (fn b_char-fn [parser]
         (when DEBUG (debug-rule "b_char"))
         (body parser))
       nil))))
;; [027]
;; nb-char ::=
;;   c-printable - b-char - c-byte-order-mark
(def nb_char
  (let [body (p/chars nil [0x9 0x9 0x20 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF])]
    (p/leaf*
     (name* "nb_char"
       (fn nb_char-fn [parser]
         (when DEBUG (debug-rule "nb_char"))
         (body parser))
       nil))))
;; [028]
;; b-break ::=
;;   ( b-carriage-return b-line-feed )
;;   | b-carriage-return
;;   | b-line-feed
(def b_break
  (let [body (p/brk nil)]
    (p/leaf*
     (name* "b_break"
       (fn b_break-fn [parser]
         (when DEBUG (debug-rule "b_break"))
         (body parser))
       nil))))
;; [029]
;; b-as-line-feed ::=
;;   b-break
(def b_as_line_feed
  (let [body (delay
               (let [parser nil]
    b_break))]
    (name* "b_as_line_feed"
      (fn b_as_line_feed-fn [parser]
        (when DEBUG (debug-rule "b_as_line_feed"))
        @body)
      nil)))
;; [030]
;; b-non-content ::=
;;   b-break
(def b_non_content
  (let [body (delay
               (let [parser nil]
    b_break))]
    (name* "b_non_content"
      (fn b_non_content-fn [parser]
        (when DEBUG (debug-rule "b_non_content"))
        @body)
      nil)))
;; [031]
;; s-space ::=
;;   x:20
(def s_space
  (let [body (p/chr nil "\u0020")]
    (p/leaf*
     (name* "s_space"
       (fn s_space-fn [parser]
         (when DEBUG (debug-rule "s_space"))
         (body parser))
       nil))))
;; [032]
;; s-tab ::=
;;   x:9
(def s_tab
  (let [body (p/chr nil "\u0009")]
    (p/leaf*
     (name* "s_tab"
       (fn s_tab-fn [parser]
         (when DEBUG (debug-rule "s_tab"))
         (body parser))
       nil))))
;; [033]
;; s-white ::=
;;   s-space | s-tab
(def s_white
  (let [body (p/chars nil [0x9 0x9 0x20 0x20])]
    (name* "s_white"
      (fn s_white-fn [parser]
        (when DEBUG (debug-rule "s_white"))
        (body parser))
      nil)))

;; [034]
;; ns-char ::=
;;   nb-char - s-white
(def ns_char
  (let [body (p/chars nil [0x21 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF])]
    (name* "ns_char"
      (fn ns_char-fn [parser]
        (when DEBUG (debug-rule "ns_char"))
        (body parser))
      nil)))

;; [035]
;; ns-dec-digit ::=
;;   [x:30-x:39]
(def ns_dec_digit
  (let [body (p/rng nil "\u0030" "\u0039")]
    (p/leaf*
     (name* "ns_dec_digit"
       (fn ns_dec_digit-fn [parser]
         (when DEBUG (debug-rule "ns_dec_digit"))
         (body parser))
       nil))))
;; [036]
;; ns-hex-digit ::=
;;   ns-dec-digit
;;   | [x:41-x:46] | [x:61-x:66]
(def ns_hex_digit
  (let [body (p/chars nil [0x30 0x39 0x41 0x46 0x61 0x66])]
    (p/leaf*
     (name* "ns_hex_digit"
       (fn ns_hex_digit-fn [parser]
         (when DEBUG (debug-rule "ns_hex_digit"))
         (body parser))
       nil))))
;; [037]
;; ns-ascii-letter ::=
;;   [x:41-x:5A] | [x:61-x:7A]
(def ns_ascii_letter
  (let [body (p/chars nil [0x41 0x5A 0x61 0x7A])]
    (p/leaf*
     (name* "ns_ascii_letter"
       (fn ns_ascii_letter-fn [parser]
         (when DEBUG (debug-rule "ns_ascii_letter"))
         (body parser))
       nil))))
;; [038]
;; ns-word-char ::=
;;   ns-dec-digit | ns-ascii-letter | '-'
(def ns_word_char
  (let [body (p/chars nil [0x2D 0x2D 0x30 0x39 0x41 0x5A 0x61 0x7A])]
    (p/leaf*
     (name* "ns_word_char"
       (fn ns_word_char-fn [parser]
         (when DEBUG (debug-rule "ns_word_char"))
         (body parser))
       nil))))
;; [039]
;; ns-uri-char ::=
;;   '%' ns-hex-digit ns-hex-digit | ns-word-char | '#'
;;   | ';' | '/' | '?' | ':' | '@' | '&' | '=' | '+' | '$' | ','
;;   | '_' | '.' | '!' | '~' | '*' | ''' | '(' | ')' | '[' | ']'
(def ns_uri_char
  (let [body (delay
               (let [parser nil]
    (p/any parser
     (p/all parser
       (p/chr parser
         "%"
      ),
      ns_hex_digit,
      ns_hex_digit
    ),
    ns_word_char,
    (p/chr parser
       "\u0023"
    ),
    (p/chr parser
       ";"
    ),
    (p/chr parser
       "/"
    ),
    (p/chr parser
       "?"
    ),
    (p/chr parser
       ":"
    ),
    (p/chr parser
       "\u0040"
    ),
    (p/chr parser
       "&"
    ),
    (p/chr parser
       "="
    ),
    (p/chr parser
       "+"
    ),
    (p/chr parser
       "$"
    ),
    (p/chr parser
       ","
    ),
    (p/chr parser
       "_"
    ),
    (p/chr parser
       "."
    ),
    (p/chr parser
       "!"
    ),
    (p/chr parser
       "~"
    ),
    (p/chr parser
       "*"
    ),
    (p/chr parser
       "'"
    ),
    (p/chr parser
       "("
    ),
    (p/chr parser
       ")"
    ),
    (p/chr parser
       "["
    ),
    (p/chr parser
       "]"
    )
  )))]
    (name* "ns_uri_char"
      (fn ns_uri_char-fn [parser]
        (when DEBUG (debug-rule "ns_uri_char"))
        @body)
      nil)))
;; [040]
;; ns-tag-char ::=
;;   ns-uri-char - '!' - c-flow-indicator
(def ns_tag_char
  (let [body (delay
               (let [parser nil]
    (p/but parser
     ns_uri_char,
    (p/chr parser
       "!"
    ),
    c_flow_indicator
  )))]
    (name* "ns_tag_char"
      (fn ns_tag_char-fn [parser]
        (when DEBUG (debug-rule "ns_tag_char"))
        @body)
      nil)))
;; [041]
;; c-escape ::=
;;   '\\'
(def c_escape
  (let [body (p/chr nil "\u005C")]
    (p/leaf*
     (name* "c_escape"
       (fn c_escape-fn [parser]
         (when DEBUG (debug-rule "c_escape"))
         (body parser))
       nil))))
;; [042]
;; ns-esc-null ::=
;;   '0'
(def ns_esc_null
  (let [body (p/chr nil "\u0030")]
    (p/leaf*
     (name* "ns_esc_null"
       (fn ns_esc_null-fn [parser]
         (when DEBUG (debug-rule "ns_esc_null"))
         (body parser))
       nil))))
;; [043]
;; ns-esc-bell ::=
;;   'a'
(def ns_esc_bell
  (let [body (p/chr nil "\u0061")]
    (p/leaf*
     (name* "ns_esc_bell"
       (fn ns_esc_bell-fn [parser]
         (when DEBUG (debug-rule "ns_esc_bell"))
         (body parser))
       nil))))
;; [044]
;; ns-esc-backspace ::=
;;   'b'
(def ns_esc_backspace
  (let [body (p/chr nil "\u0062")]
    (p/leaf*
     (name* "ns_esc_backspace"
       (fn ns_esc_backspace-fn [parser]
         (when DEBUG (debug-rule "ns_esc_backspace"))
         (body parser))
       nil))))
;; [045]
;; ns-esc-horizontal-tab ::=
;;   't' | x:9
(def ns_esc_horizontal_tab
  (let [body (p/chars nil [0x9 0x9 0x74 0x74])]
    (p/leaf*
     (name* "ns_esc_horizontal_tab"
       (fn ns_esc_horizontal_tab-fn [parser]
         (when DEBUG (debug-rule "ns_esc_horizontal_tab"))
         (body parser))
       nil))))
;; [046]
;; ns-esc-line-feed ::=
;;   'n'
(def ns_esc_line_feed
  (let [body (p/chr nil "\u006E")]
    (p/leaf*
     (name* "ns_esc_line_feed"
       (fn ns_esc_line_feed-fn [parser]
         (when DEBUG (debug-rule "ns_esc_line_feed"))
         (body parser))
       nil))))
;; [047]
;; ns-esc-vertical-tab ::=
;;   'v'
(def ns_esc_vertical_tab
  (let [body (p/chr nil "\u0076")]
    (p/leaf*
     (name* "ns_esc_vertical_tab"
       (fn ns_esc_vertical_tab-fn [parser]
         (when DEBUG (debug-rule "ns_esc_vertical_tab"))
         (body parser))
       nil))))
;; [048]
;; ns-esc-form-feed ::=
;;   'f'
(def ns_esc_form_feed
  (let [body (p/chr nil "\u0066")]
    (p/leaf*
     (name* "ns_esc_form_feed"
       (fn ns_esc_form_feed-fn [parser]
         (when DEBUG (debug-rule "ns_esc_form_feed"))
         (body parser))
       nil))))
;; [049]
;; ns-esc-carriage-return ::=
;;   'r'
(def ns_esc_carriage_return
  (let [body (p/chr nil "\u0072")]
    (p/leaf*
     (name* "ns_esc_carriage_return"
       (fn ns_esc_carriage_return-fn [parser]
         (when DEBUG (debug-rule "ns_esc_carriage_return"))
         (body parser))
       nil))))
;; [050]
;; ns-esc-escape ::=
;;   'e'
(def ns_esc_escape
  (let [body (p/chr nil "\u0065")]
    (p/leaf*
     (name* "ns_esc_escape"
       (fn ns_esc_escape-fn [parser]
         (when DEBUG (debug-rule "ns_esc_escape"))
         (body parser))
       nil))))
;; [051]
;; ns-esc-space ::=
;;   x:20
(def ns_esc_space
  (let [body (p/chr nil "\u0020")]
    (p/leaf*
     (name* "ns_esc_space"
       (fn ns_esc_space-fn [parser]
         (when DEBUG (debug-rule "ns_esc_space"))
         (body parser))
       nil))))
;; [052]
;; ns-esc-double-quote ::=
;;   '"'
(def ns_esc_double_quote
  (let [body (p/chr nil "\u0022")]
    (p/leaf*
     (name* "ns_esc_double_quote"
       (fn ns_esc_double_quote-fn [parser]
         (when DEBUG (debug-rule "ns_esc_double_quote"))
         (body parser))
       nil))))
;; [053]
;; ns-esc-slash ::=
;;   '/'
(def ns_esc_slash
  (let [body (p/chr nil "\u002F")]
    (p/leaf*
     (name* "ns_esc_slash"
       (fn ns_esc_slash-fn [parser]
         (when DEBUG (debug-rule "ns_esc_slash"))
         (body parser))
       nil))))
;; [054]
;; ns-esc-backslash ::=
;;   '\\'
(def ns_esc_backslash
  (let [body (p/chr nil "\u005C")]
    (p/leaf*
     (name* "ns_esc_backslash"
       (fn ns_esc_backslash-fn [parser]
         (when DEBUG (debug-rule "ns_esc_backslash"))
         (body parser))
       nil))))
;; [055]
;; ns-esc-next-line ::=
;;   'N'
(def ns_esc_next_line
  (let [body (p/chr nil "\u004E")]
    (p/leaf*
     (name* "ns_esc_next_line"
       (fn ns_esc_next_line-fn [parser]
         (when DEBUG (debug-rule "ns_esc_next_line"))
         (body parser))
       nil))))
;; [056]
;; ns-esc-non-breaking-space ::=
;;   '_'
(def ns_esc_non_breaking_space
  (let [body (p/chr nil "\u005F")]
    (p/leaf*
     (name* "ns_esc_non_breaking_space"
       (fn ns_esc_non_breaking_space-fn [parser]
         (when DEBUG (debug-rule "ns_esc_non_breaking_space"))
         (body parser))
       nil))))
;; [057]
;; ns-esc-line-separator ::=
;;   'L'
(def ns_esc_line_separator
  (let [body (p/chr nil "\u004C")]
    (p/leaf*
     (name* "ns_esc_line_separator"
       (fn ns_esc_line_separator-fn [parser]
         (when DEBUG (debug-rule "ns_esc_line_separator"))
         (body parser))
       nil))))
;; [058]
;; ns-esc-paragraph-separator ::=
;;   'P'
(def ns_esc_paragraph_separator
  (let [body (p/chr nil "\u0050")]
    (p/leaf*
     (name* "ns_esc_paragraph_separator"
       (fn ns_esc_paragraph_separator-fn [parser]
         (when DEBUG (debug-rule "ns_esc_paragraph_separator"))
         (body parser))
       nil))))
;; [059]
;; ns-esc-8-bit ::=
;;   'x'
;;   ( ns-hex-digit{2} )
(def ns_esc_8_bit
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "x"
    ),
    (p/chars-rep parser 2, 2, [0x30 0x39 0x41 0x46 0x61 0x66])
  )))]
    (name* "ns_esc_8_bit"
      (fn ns_esc_8_bit-fn [parser]
        (when DEBUG (debug-rule "ns_esc_8_bit"))
        @body)
      nil)))
;; [060]
;; ns-esc-16-bit ::=
;;   'u'
;;   ( ns-hex-digit{4} )
(def ns_esc_16_bit
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "u"
    ),
    (p/chars-rep parser 4, 4, [0x30 0x39 0x41 0x46 0x61 0x66])
  )))]
    (name* "ns_esc_16_bit"
      (fn ns_esc_16_bit-fn [parser]
        (when DEBUG (debug-rule "ns_esc_16_bit"))
        @body)
      nil)))
;; [061]
;; ns-esc-32-bit ::=
;;   'U'
;;   ( ns-hex-digit{8} )
(def ns_esc_32_bit
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "U"
    ),
    (p/chars-rep parser 8, 8, [0x30 0x39 0x41 0x46 0x61 0x66])
  )))]
    (name* "ns_esc_32_bit"
      (fn ns_esc_32_bit-fn [parser]
        (when DEBUG (debug-rule "ns_esc_32_bit"))
        @body)
      nil)))
;; [062]
;; c-ns-esc-char ::=
;;   '\\'
;;   ( ns-esc-null | ns-esc-bell | ns-esc-backspace
;;   | ns-esc-horizontal-tab | ns-esc-line-feed
;;   | ns-esc-vertical-tab | ns-esc-form-feed
;;   | ns-esc-carriage-return | ns-esc-escape | ns-esc-space
;;   | ns-esc-double-quote | ns-esc-slash | ns-esc-backslash
;;   | ns-esc-next-line | ns-esc-non-breaking-space
;;   | ns-esc-line-separator | ns-esc-paragraph-separator
;;   | ns-esc-8-bit | ns-esc-16-bit | ns-esc-32-bit )
(def c_ns_esc_char
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "\\"
    ),
    (p/any parser
       ns_esc_null,
      ns_esc_bell,
      ns_esc_backspace,
      ns_esc_horizontal_tab,
      ns_esc_line_feed,
      ns_esc_vertical_tab,
      ns_esc_form_feed,
      ns_esc_carriage_return,
      ns_esc_escape,
      ns_esc_space,
      ns_esc_double_quote,
      ns_esc_slash,
      ns_esc_backslash,
      ns_esc_next_line,
      ns_esc_non_breaking_space,
      ns_esc_line_separator,
      ns_esc_paragraph_separator,
      ns_esc_8_bit,
      ns_esc_16_bit,
      ns_esc_32_bit
    )
  )))]
    (name* "c_ns_esc_char"
      (fn c_ns_esc_char-fn [parser]
        (when DEBUG (debug-rule "c_ns_esc_char"))
        @body)
      nil)))
;; [063]
;; s-indent(n) ::=
;;   s-space{n}
(def s_indent
  (let [cache (volatile! {})]
    (name* "s_indent"
      (fn s_indent-fn [parser n]
        (when DEBUG (debug-rule "s_indent", n))
        (or (get @cache [n])
            (let [body
  (p/chars-rep parser n, n, [0x20 0x20])]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [064]
;; s-indent(<n) ::=
;;   s-space{m} <where_m_<_n>
(def s_indent_lt
  (let [cache (volatile! {})]
    (name* "s_indent_lt"
      (fn s_indent_lt-fn [parser n]
        (when DEBUG (debug-rule "s_indent_lt", n))
        (or (get @cache [n])
            (let [body (p/indent-cmp parser (pos? n))]
              (vswap! cache assoc [n] body)
              body)))
      nil)))

;; [065]
;; s-indent(<=n) ::=
;;   s-space{m} <where_m_<=_n>
(def s_indent_le
  (let [cache (volatile! {})]
    (name* "s_indent_le"
      (fn s_indent_le-fn [parser n]
        (when DEBUG (debug-rule "s_indent_le", n))
        (or (get @cache [n])
            (let [body (p/indent-cmp parser (not (neg? n)))]
              (vswap! cache assoc [n] body)
              body)))
      nil)))

;; [066]
;; s-separate-in-line ::=
;;   s-white+ | <start_of_line>
(def s_separate_in_line
  (let [body (p/sep-in-line nil)]
    (p/leaf*
     (name* "s_separate_in_line"
       (fn s_separate_in_line-fn [parser]
         (when DEBUG (debug-rule "s_separate_in_line"))
         (body parser))
       nil))))
;; [067]
;; s-line-prefix(n,c) ::=
;;   ( c = block-out => s-block-line-prefix(n) )
;;   ( c = block-in => s-block-line-prefix(n) )
;;   ( c = flow-out => s-flow-line-prefix(n) )
;;   ( c = flow-in => s-flow-line-prefix(n) )
(def s_line_prefix
  (let [cache (volatile! {})]
    (name* "s_line_prefix"
      (fn s_line_prefix-fn [parser n c]
        (when DEBUG (debug-rule "s_line_prefix", n, c))
        (or (get @cache [n c])
            (let [body
  (p/case-at parser
     c,
    {
      "block-in" [s_block_line_prefix, n],
      "block-out" [s_block_line_prefix, n],
      "flow-in" [s_flow_line_prefix, n],
      "flow-out" [s_flow_line_prefix, n],
    }
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [068]
;; s-block-line-prefix(n) ::=
;;   s-indent(n)
(def s_block_line_prefix
  (let [cache (volatile! {})]
    (name* "s_block_line_prefix"
      (fn s_block_line_prefix-fn [parser n]
        (when DEBUG (debug-rule "s_block_line_prefix", n))
        (or (get @cache [n])
            (let [body
  [s_indent, n]]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [069]
;; s-flow-line-prefix(n) ::=
;;   s-indent(n)
;;   s-separate-in-line?
(def s_flow_line_prefix
  (let [cache (volatile! {})]
    (name* "s_flow_line_prefix"
      (fn s_flow_line_prefix-fn [parser n]
        (when DEBUG (debug-rule "s_flow_line_prefix", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [s_indent, n],
    (p/rep parser
       0,
      1,
      s_separate_in_line
    )
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [070]
;; l-empty(n,c) ::=
;;   ( s-line-prefix(n,c) | s-indent(<n) )
;;   b-as-line-feed
(def l_empty
  (let [cache (volatile! {})]
    (name* "l_empty"
      (fn l_empty-fn [parser n c]
        (when DEBUG (debug-rule "l_empty", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     (p/any parser
       [s_line_prefix, n, c],
      [s_indent_lt, n]
    ),
    b_as_line_feed
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [071]
;; b-l-trimmed(n,c) ::=
;;   b-non-content l-empty(n,c)+
(def b_l_trimmed
  (let [cache (volatile! {})]
    (name* "b_l_trimmed"
      (fn b_l_trimmed-fn [parser n c]
        (when DEBUG (debug-rule "b_l_trimmed", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     b_non_content,
    (p/rep parser
       1,
      nil,
      [l_empty, n, c]
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [072]
;; b-as-space ::=
;;   b-break
(def b_as_space
  (let [body (delay
               (let [parser nil]
    b_break))]
    (name* "b_as_space"
      (fn b_as_space-fn [parser]
        (when DEBUG (debug-rule "b_as_space"))
        @body)
      nil)))
;; [073]
;; b-l-folded(n,c) ::=
;;   b-l-trimmed(n,c) | b-as-space
(def b_l_folded
  (let [cache (volatile! {})]
    (name* "b_l_folded"
      (fn b_l_folded-fn [parser n c]
        (when DEBUG (debug-rule "b_l_folded", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     [b_l_trimmed, n, c],
    b_as_space
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [074]
;; s-flow-folded(n) ::=
;;   s-separate-in-line?
;;   b-l-folded(n,flow-in)
;;   s-flow-line-prefix(n)
(def s_flow_folded
  (let [cache (volatile! {})]
    (name* "s_flow_folded"
      (fn s_flow_folded-fn [parser n]
        (when DEBUG (debug-rule "s_flow_folded", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/rep parser
       0,
      1,
      s_separate_in_line
    ),
    [b_l_folded, n, "flow-in"],
    [s_flow_line_prefix, n]
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [075]
;; c-nb-comment-text ::=
;;   '#' nb-char*
(def c_nb_comment_text
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "\u0023"
    ),
    (p/chars-rep parser 0, nil, [0x9 0x9 0x20 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF])
  )))]
    (name* "c_nb_comment_text"
      (fn c_nb_comment_text-fn [parser]
        (when DEBUG (debug-rule "c_nb_comment_text"))
        @body)
      nil)))
;; [076]
;; b-comment ::=
;;   b-non-content | <end_of_file>
(def b_comment
  (let [body (delay
               (let [parser nil]
    (p/any parser
     b_non_content,
    (p/end-of-stream parser)
  )))]
    (name* "b_comment"
      (fn b_comment-fn [parser]
        (when DEBUG (debug-rule "b_comment"))
        @body)
      nil)))
;; [077]
;; s-b-comment ::=
;;   ( s-separate-in-line
;;   c-nb-comment-text? )?
;;   b-comment
(def s_b_comment
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/rep parser
       0,
      1,
      (p/all parser
         s_separate_in_line,
        (p/rep parser
           0,
          1,
          c_nb_comment_text
        )
      )),
    b_comment
  )))]
    (name* "s_b_comment"
      (fn s_b_comment-fn [parser]
        (when DEBUG (debug-rule "s_b_comment"))
        @body)
      nil)))
;; [078]
;; l-comment ::=
;;   s-separate-in-line c-nb-comment-text?
;;   b-comment
(def l_comment
  (let [body (delay
               (let [parser nil]
    (p/all parser
     s_separate_in_line,
    (p/rep parser
       0,
      1,
      c_nb_comment_text
    ),
    b_comment
  )))]
    (name* "l_comment"
      (fn l_comment-fn [parser]
        (when DEBUG (debug-rule "l_comment"))
        @body)
      nil)))
;; [079]
;; s-l-comments ::=
;;   ( s-b-comment | <start_of_line> )
;;   l-comment*
(def s_l_comments
  (let [body (p/comments-scan nil [0x9 0x9 0x20 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF])]
    (p/leaf*
     (name* "s_l_comments"
       (fn s_l_comments-fn [parser]
         (when DEBUG (debug-rule "s_l_comments"))
         (body parser))
       nil))))
;; [080]
;; s-separate(n,c) ::=
;;   ( c = block-out => s-separate-lines(n) )
;;   ( c = block-in => s-separate-lines(n) )
;;   ( c = flow-out => s-separate-lines(n) )
;;   ( c = flow-in => s-separate-lines(n) )
;;   ( c = block-key => s-separate-in-line )
;;   ( c = flow-key => s-separate-in-line )
(def s_separate
  (let [cache (volatile! {})]
    (name* "s_separate"
      (fn s_separate-fn [parser n c]
        (when DEBUG (debug-rule "s_separate", n, c))
        (or (get @cache [n c])
            (let [body
  (p/case-at parser
     c,
    {
      "block-in" [s_separate_lines, n],
      "block-key" s_separate_in_line,
      "block-out" [s_separate_lines, n],
      "flow-in" [s_separate_lines, n],
      "flow-key" s_separate_in_line,
      "flow-out" [s_separate_lines, n],
    }
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [081]
;; s-separate-lines(n) ::=
;;   ( s-l-comments
;;   s-flow-line-prefix(n) )
;;   | s-separate-in-line
(def s_separate_lines
  (let [cache (volatile! {})]
    (name* "s_separate_lines"
      (fn s_separate_lines-fn [parser n]
        (when DEBUG (debug-rule "s_separate_lines", n))
        (or (get @cache [n])
            (let [body (p/sep-lines parser n [0x9 0x9 0x20 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF])]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [082]
;; l-directive ::=
;;   '%'
;;   ( ns-yaml-directive
;;   | ns-tag-directive
;;   | ns-reserved-directive )
;;   s-l-comments
(def l_directive
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "%"
    ),
    (p/any parser
       ns_yaml_directive,
      ns_tag_directive,
      ns_reserved_directive
    ),
    s_l_comments
  )))]
    (name* "l_directive"
      (fn l_directive-fn [parser]
        (when DEBUG (debug-rule "l_directive"))
        @body)
      nil)))
;; [083]
;; ns-reserved-directive ::=
;;   ns-directive-name
;;   ( s-separate-in-line ns-directive-parameter )*
(def ns_reserved_directive
  (let [body (delay
               (let [parser nil]
    (p/all parser
     ns_directive_name,
    (p/rep parser
       0,
      nil,
      (p/all parser
         s_separate_in_line,
        ns_directive_parameter
      ))
  )))]
    (name* "ns_reserved_directive"
      (fn ns_reserved_directive-fn [parser]
        (when DEBUG (debug-rule "ns_reserved_directive"))
        @body)
      nil)))
;; [084]
;; ns-directive-name ::=
;;   ns-char+
(def ns_directive_name
  (let [body (delay
               (let [parser nil]
    (p/rep2 parser 1, nil, ns_char)))]
    (name* "ns_directive_name"
      (fn ns_directive_name-fn [parser]
        (when DEBUG (debug-rule "ns_directive_name"))
        @body)
      nil)))
;; [085]
;; ns-directive-parameter ::=
;;   ns-char+
(def ns_directive_parameter
  (let [body (delay
               (let [parser nil]
    (p/rep2 parser 1, nil, ns_char)))]
    (name* "ns_directive_parameter"
      (fn ns_directive_parameter-fn [parser]
        (when DEBUG (debug-rule "ns_directive_parameter"))
        @body)
      nil)))
;; [086]
;; ns-yaml-directive ::=
;;   'Y' 'A' 'M' 'L'
;;   s-separate-in-line ns-yaml-version
(def ns_yaml_directive
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "Y"
    ),
    (p/chr parser
       "A"
    ),
    (p/chr parser
       "M"
    ),
    (p/chr parser
       "L"
    ),
    s_separate_in_line,
    ns_yaml_version
  )))]
    (name* "ns_yaml_directive"
      (fn ns_yaml_directive-fn [parser]
        (when DEBUG (debug-rule "ns_yaml_directive"))
        @body)
      nil)))
;; [087]
;; ns-yaml-version ::=
;;   ns-dec-digit+ '.' ns-dec-digit+
(def ns_yaml_version
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chars-rep parser 1, nil, [0x30 0x39]),
    (p/chr parser
       "."
    ),
    (p/chars-rep parser 1, nil, [0x30 0x39])
  )))]
    (name* "ns_yaml_version"
      (fn ns_yaml_version-fn [parser]
        (when DEBUG (debug-rule "ns_yaml_version"))
        @body)
      nil)))
;; [088]
;; ns-tag-directive ::=
;;   'T' 'A' 'G'
;;   s-separate-in-line c-tag-handle
;;   s-separate-in-line ns-tag-prefix
(def ns_tag_directive
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "T"
    ),
    (p/chr parser
       "A"
    ),
    (p/chr parser
       "G"
    ),
    s_separate_in_line,
    c_tag_handle,
    s_separate_in_line,
    ns_tag_prefix
  )))]
    (name* "ns_tag_directive"
      (fn ns_tag_directive-fn [parser]
        (when DEBUG (debug-rule "ns_tag_directive"))
        @body)
      nil)))
;; [089]
;; c-tag-handle ::=
;;   c-named-tag-handle
;;   | c-secondary-tag-handle
;;   | c-primary-tag-handle
(def c_tag_handle
  (let [body (delay
               (let [parser nil]
    (p/any parser
     c_named_tag_handle,
    c_secondary_tag_handle,
    c_primary_tag_handle
  )))]
    (name* "c_tag_handle"
      (fn c_tag_handle-fn [parser]
        (when DEBUG (debug-rule "c_tag_handle"))
        @body)
      nil)))
;; [090]
;; c-primary-tag-handle ::=
;;   '!'
(def c_primary_tag_handle
  (let [body (p/chr nil "\u0021")]
    (p/leaf*
     (name* "c_primary_tag_handle"
       (fn c_primary_tag_handle-fn [parser]
         (when DEBUG (debug-rule "c_primary_tag_handle"))
         (body parser))
       nil))))
;; [091]
;; c-secondary-tag-handle ::=
;;   '!' '!'
(def c_secondary_tag_handle
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "!"
    ),
    (p/chr parser
       "!"
    )
  )))]
    (name* "c_secondary_tag_handle"
      (fn c_secondary_tag_handle-fn [parser]
        (when DEBUG (debug-rule "c_secondary_tag_handle"))
        @body)
      nil)))
;; [092]
;; c-named-tag-handle ::=
;;   '!' ns-word-char+ '!'
(def c_named_tag_handle
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "!"
    ),
    (p/chars-rep parser 1, nil, [0x2D 0x2D 0x30 0x39 0x41 0x5A 0x61 0x7A]),
    (p/chr parser
       "!"
    )
  )))]
    (name* "c_named_tag_handle"
      (fn c_named_tag_handle-fn [parser]
        (when DEBUG (debug-rule "c_named_tag_handle"))
        @body)
      nil)))
;; [093]
;; ns-tag-prefix ::=
;;   c-ns-local-tag-prefix | ns-global-tag-prefix
(def ns_tag_prefix
  (let [body (delay
               (let [parser nil]
    (p/any parser
     c_ns_local_tag_prefix,
    ns_global_tag_prefix
  )))]
    (name* "ns_tag_prefix"
      (fn ns_tag_prefix-fn [parser]
        (when DEBUG (debug-rule "ns_tag_prefix"))
        @body)
      nil)))
;; [094]
;; c-ns-local-tag-prefix ::=
;;   '!' ns-uri-char*
(def c_ns_local_tag_prefix
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "!"
    ),
    (p/rep parser
       0,
      nil,
      ns_uri_char
    )
  )))]
    (name* "c_ns_local_tag_prefix"
      (fn c_ns_local_tag_prefix-fn [parser]
        (when DEBUG (debug-rule "c_ns_local_tag_prefix"))
        @body)
      nil)))
;; [095]
;; ns-global-tag-prefix ::=
;;   ns-tag-char ns-uri-char*
(def ns_global_tag_prefix
  (let [body (delay
               (let [parser nil]
    (p/all parser
     ns_tag_char,
    (p/rep parser
       0,
      nil,
      ns_uri_char
    )
  )))]
    (name* "ns_global_tag_prefix"
      (fn ns_global_tag_prefix-fn [parser]
        (when DEBUG (debug-rule "ns_global_tag_prefix"))
        @body)
      nil)))
;; [096]
;; c-ns-properties(n,c) ::=
;;   ( c-ns-tag-property
;;   ( s-separate(n,c) c-ns-anchor-property )? )
;;   | ( c-ns-anchor-property
;;   ( s-separate(n,c) c-ns-tag-property )? )
(def c_ns_properties
  (let [cache (volatile! {})]
    (name* "c_ns_properties"
      (fn c_ns_properties-fn [parser n c]
        (when DEBUG (debug-rule "c_ns_properties", n, c))
        (if (p/ahead? parser [0x21 0x21 0x26 0x26])
          (or (get @cache [n c])
              (let [body
  (p/any parser
     (p/all parser
       c_ns_tag_property,
      (p/rep parser
         0,
        1,
        (p/all parser
           [s_separate, n, c],
          c_ns_anchor_property
        ))
    ),
    (p/all parser
       c_ns_anchor_property,
      (p/rep parser
         0,
        1,
        (p/all parser
           [s_separate, n, c],
          c_ns_tag_property
        ))
    )
  )]
                (vswap! cache assoc [n c] body)
                body))
          false))
      nil)))
;; [097]
;; c-ns-tag-property ::=
;;   c-verbatim-tag
;;   | c-ns-shorthand-tag
;;   | c-non-specific-tag
(def c_ns_tag_property
  (let [body (delay
               (let [parser nil]
    (p/any parser
     c_verbatim_tag,
    c_ns_shorthand_tag,
    c_non_specific_tag
  )))]
    (name* "c_ns_tag_property"
      (fn c_ns_tag_property-fn [parser]
        (when DEBUG (debug-rule "c_ns_tag_property"))
        (if (p/ahead? parser [0x21 0x21])
          @body
          false))
      nil)))
;; [098]
;; c-verbatim-tag ::=
;;   '!' '<' ns-uri-char+ '>'
(def c_verbatim_tag
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "!"
    ),
    (p/chr parser
       "<"
    ),
    (p/rep parser
       1,
      nil,
      ns_uri_char
    ),
    (p/chr parser
       ">"
    )
  )))]
    (name* "c_verbatim_tag"
      (fn c_verbatim_tag-fn [parser]
        (when DEBUG (debug-rule "c_verbatim_tag"))
        @body)
      nil)))
;; [099]
;; c-ns-shorthand-tag ::=
;;   c-tag-handle ns-tag-char+
(def c_ns_shorthand_tag
  (let [body (delay
               (let [parser nil]
    (p/all parser
     c_tag_handle,
    (p/rep parser
       1,
      nil,
      ns_tag_char
    )
  )))]
    (name* "c_ns_shorthand_tag"
      (fn c_ns_shorthand_tag-fn [parser]
        (when DEBUG (debug-rule "c_ns_shorthand_tag"))
        @body)
      nil)))
;; [100]
;; c-non-specific-tag ::=
;;   '!'
(def c_non_specific_tag
  (let [body (p/chr nil "\u0021")]
    (p/leaf*
     (name* "c_non_specific_tag"
       (fn c_non_specific_tag-fn [parser]
         (when DEBUG (debug-rule "c_non_specific_tag"))
         (body parser))
       nil))))
;; [101]
;; c-ns-anchor-property ::=
;;   '&' ns-anchor-name
(def c_ns_anchor_property
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "&"
    ),
    ns_anchor_name
  )))]
    (name* "c_ns_anchor_property"
      (fn c_ns_anchor_property-fn [parser]
        (when DEBUG (debug-rule "c_ns_anchor_property"))
        (if (p/ahead? parser [0x26 0x26])
          @body
          false))
      nil)))
;; [102]
;; ns-anchor-char ::=
;;   ns-char - c-flow-indicator
(def ns_anchor_char
  (let [body (delay
               (let [parser nil]
    (p/but parser
     ns_char,
    c_flow_indicator
  )))]
    (name* "ns_anchor_char"
      (fn ns_anchor_char-fn [parser]
        (when DEBUG (debug-rule "ns_anchor_char"))
        @body)
      nil)))
;; [103]
;; ns-anchor-name ::=
;;   ns-anchor-char+
(def ns_anchor_name
  (let [body (delay
               (let [parser nil]
    (p/rep2 parser 1, nil, ns_anchor_char)))]
    (name* "ns_anchor_name"
      (fn ns_anchor_name-fn [parser]
        (when DEBUG (debug-rule "ns_anchor_name"))
        @body)
      nil)))
;; [104]
;; c-ns-alias-node ::=
;;   '*' ns-anchor-name
(def c_ns_alias_node
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "*"
    ),
    ns_anchor_name
  )))]
    (name* "c_ns_alias_node"
      (fn c_ns_alias_node-fn [parser]
        (when DEBUG (debug-rule "c_ns_alias_node"))
        (if (p/ahead? parser [0x2A 0x2A])
          @body
          false))
      nil)))
;; [105]
;; e-scalar ::=
;;   <empty>
(def e_scalar
  (let [body (delay
               (let [parser nil]
    empty))]
    (name* "e_scalar"
      (fn e_scalar-fn [parser]
        (when DEBUG (debug-rule "e_scalar"))
        @body)
      nil)))
;; [106]
;; e-node ::=
;;   e-scalar
(def e_node
  (let [body (delay
               (let [parser nil]
    e_scalar))]
    (name* "e_node"
      (fn e_node-fn [parser]
        (when DEBUG (debug-rule "e_node"))
        @body)
      nil)))
;; [107]
;; nb-double-char ::=
;;   c-ns-esc-char | ( nb-json - '\\' - '"' )
(def nb_double_char
  (let [body (delay
               (let [parser nil]
    (p/any parser
     c_ns_esc_char,
    (p/chars parser [0x9 0x9 0x20 0x21 0x23 0x5B 0x5D 0x10FFFF])
  )))]
    (name* "nb_double_char"
      (fn nb_double_char-fn [parser]
        (when DEBUG (debug-rule "nb_double_char"))
        @body)
      nil)))
;; [108]
;; ns-double-char ::=
;;   nb-double-char - s-white
(def ns_double_char
  (let [body (delay
               (let [parser nil]
    (p/but parser
     nb_double_char,
    s_white
  )))]
    (name* "ns_double_char"
      (fn ns_double_char-fn [parser]
        (when DEBUG (debug-rule "ns_double_char"))
        @body)
      nil)))
;; [109]
;; c-double-quoted(n,c) ::=
;;   '"' nb-double-text(n,c)
;;   '"'
(def c_double_quoted
  (let [cache (volatile! {})]
    (name* "c_double_quoted"
      (fn c_double_quoted-fn [parser n c]
        (when DEBUG (debug-rule "c_double_quoted", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     (p/chr parser
       "\""
    ),
    [nb_double_text, n, c],
    (p/chr parser
       "\""
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [110]
;; nb-double-text(n,c) ::=
;;   ( c = flow-out => nb-double-multi-line(n) )
;;   ( c = flow-in => nb-double-multi-line(n) )
;;   ( c = block-key => nb-double-one-line )
;;   ( c = flow-key => nb-double-one-line )
(def nb_double_text
  (let [cache (volatile! {})]
    (name* "nb_double_text"
      (fn nb_double_text-fn [parser n c]
        (when DEBUG (debug-rule "nb_double_text", n, c))
        (or (get @cache [n c])
            (let [body
  (p/case-at parser
     c,
    {
      "block-key" nb_double_one_line,
      "flow-in" [nb_double_multi_line, n],
      "flow-key" nb_double_one_line,
      "flow-out" [nb_double_multi_line, n],
    }
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [111]
;; nb-double-one-line ::=
;;   nb-double-char*
(def nb_double_one_line
  (let [body (p/dquo-scan nil [0x9 0x9 0x20 0x21 0x23 0x5B 0x5D 0x10FFFF]
                          [0x9 0x9 0x20 0x20 0x22 0x22 0x2F 0x30 0x4C 0x4C 0x4E 0x4E 0x50 0x50 0x5C 0x5C 0x5F 0x5F 0x61 0x62 0x65 0x66 0x6E 0x6E 0x72 0x72 0x74 0x74 0x76 0x76]
                          [0x30 0x39 0x41 0x46 0x61 0x66] true)]
    (p/leaf*
     (name* "nb_double_one_line"
       (fn nb_double_one_line-fn [parser]
         (when DEBUG (debug-rule "nb_double_one_line"))
         (body parser))
       nil))))

;; [112]
;; s-double-escaped(n) ::=
;;   s-white* '\\'
;;   b-non-content
;;   l-empty(n,flow-in)* s-flow-line-prefix(n)
(def s_double_escaped
  (let [cache (volatile! {})]
    (name* "s_double_escaped"
      (fn s_double_escaped-fn [parser n]
        (when DEBUG (debug-rule "s_double_escaped", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/rep parser
       0,
      nil,
      s_white
    ),
    (p/chr parser
       "\\"
    ),
    b_non_content,
    (p/rep2 parser
       0,
      nil,
      [l_empty, n, "flow-in"]
    ),
    [s_flow_line_prefix, n]
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [113]
;; s-double-break(n) ::=
;;   s-double-escaped(n) | s-flow-folded(n)
(def s_double_break
  (let [cache (volatile! {})]
    (name* "s_double_break"
      (fn s_double_break-fn [parser n]
        (when DEBUG (debug-rule "s_double_break", n))
        (or (get @cache [n])
            (let [body
  (p/any parser
     [s_double_escaped, n],
    [s_flow_folded, n]
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [114]
;; nb-ns-double-in-line ::=
;;   ( s-white* ns-double-char )*
(def nb_ns_double_in_line
  (let [body (p/dquo-scan nil [0x9 0x9 0x20 0x21 0x23 0x5B 0x5D 0x10FFFF]
                          [0x9 0x9 0x20 0x20 0x22 0x22 0x2F 0x30 0x4C 0x4C 0x4E 0x4E 0x50 0x50 0x5C 0x5C 0x5F 0x5F 0x61 0x62 0x65 0x66 0x6E 0x6E 0x72 0x72 0x74 0x74 0x76 0x76]
                          [0x30 0x39 0x41 0x46 0x61 0x66] false)]
    (p/leaf*
     (name* "nb_ns_double_in_line"
       (fn nb_ns_double_in_line-fn [parser]
         (when DEBUG (debug-rule "nb_ns_double_in_line"))
         (body parser))
       nil))))

;; [115]
;; s-double-next-line(n) ::=
;;   s-double-break(n)
;;   ( ns-double-char nb-ns-double-in-line
;;   ( s-double-next-line(n) | s-white* ) )?
(def s_double_next_line
  (let [cache (volatile! {})]
    (name* "s_double_next_line"
      (fn s_double_next_line-fn [parser n]
        (when DEBUG (debug-rule "s_double_next_line", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [s_double_break, n],
    (p/rep parser
       0,
      1,
      (p/all parser
         ns_double_char,
        nb_ns_double_in_line,
        (p/any parser
           [s_double_next_line, n],
          (p/rep parser
             0,
            nil,
            s_white
          )
        )
      ))
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [116]
;; nb-double-multi-line(n) ::=
;;   nb-ns-double-in-line
;;   ( s-double-next-line(n) | s-white* )
(def nb_double_multi_line
  (let [cache (volatile! {})]
    (name* "nb_double_multi_line"
      (fn nb_double_multi_line-fn [parser n]
        (when DEBUG (debug-rule "nb_double_multi_line", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     nb_ns_double_in_line,
    (p/any parser
       [s_double_next_line, n],
      (p/rep parser
         0,
        nil,
        s_white
      )
    )
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [117]
;; c-quoted-quote ::=
;;   ''' '''
(def c_quoted_quote
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "'"
    ),
    (p/chr parser
       "'"
    )
  )))]
    (name* "c_quoted_quote"
      (fn c_quoted_quote-fn [parser]
        (when DEBUG (debug-rule "c_quoted_quote"))
        @body)
      nil)))
;; [118]
;; nb-single-char ::=
;;   c-quoted-quote | ( nb-json - ''' )
(def nb_single_char
  (let [body (delay
               (let [parser nil]
    (p/any parser
     c_quoted_quote,
    (p/chars parser [0x9 0x9 0x20 0x26 0x28 0x10FFFF])
  )))]
    (name* "nb_single_char"
      (fn nb_single_char-fn [parser]
        (when DEBUG (debug-rule "nb_single_char"))
        @body)
      nil)))
;; [119]
;; ns-single-char ::=
;;   nb-single-char - s-white
(def ns_single_char
  (let [body (delay
               (let [parser nil]
    (p/but parser
     nb_single_char,
    s_white
  )))]
    (name* "ns_single_char"
      (fn ns_single_char-fn [parser]
        (when DEBUG (debug-rule "ns_single_char"))
        @body)
      nil)))
;; [120]
;; c-single-quoted(n,c) ::=
;;   ''' nb-single-text(n,c)
;;   '''
(def c_single_quoted
  (let [cache (volatile! {})]
    (name* "c_single_quoted"
      (fn c_single_quoted-fn [parser n c]
        (when DEBUG (debug-rule "c_single_quoted", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     (p/chr parser
       "'"
    ),
    [nb_single_text, n, c],
    (p/chr parser
       "'"
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [121]
;; nb-single-text(n,c) ::=
;;   ( c = flow-out => nb-single-multi-line(n) )
;;   ( c = flow-in => nb-single-multi-line(n) )
;;   ( c = block-key => nb-single-one-line )
;;   ( c = flow-key => nb-single-one-line )
(def nb_single_text
  (let [cache (volatile! {})]
    (name* "nb_single_text"
      (fn nb_single_text-fn [parser n c]
        (when DEBUG (debug-rule "nb_single_text", n, c))
        (or (get @cache [n c])
            (let [body
  (p/case-at parser
     c,
    {
      "block-key" nb_single_one_line,
      "flow-in" [nb_single_multi_line, n],
      "flow-key" nb_single_one_line,
      "flow-out" [nb_single_multi_line, n],
    }
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [122]
;; nb-single-one-line ::=
;;   nb-single-char*
(def nb_single_one_line
  (let [body (p/squo-scan nil [0x9 0x9 0x20 0x26 0x28 0x10FFFF] true)]
    (p/leaf*
     (name* "nb_single_one_line"
       (fn nb_single_one_line-fn [parser]
         (when DEBUG (debug-rule "nb_single_one_line"))
         (body parser))
       nil))))

;; [123]
;; nb-ns-single-in-line ::=
;;   ( s-white* ns-single-char )*
(def nb_ns_single_in_line
  (let [body (p/squo-scan nil [0x9 0x9 0x20 0x26 0x28 0x10FFFF] false)]
    (p/leaf*
     (name* "nb_ns_single_in_line"
       (fn nb_ns_single_in_line-fn [parser]
         (when DEBUG (debug-rule "nb_ns_single_in_line"))
         (body parser))
       nil))))

;; [124]
;; s-single-next-line(n) ::=
;;   s-flow-folded(n)
;;   ( ns-single-char nb-ns-single-in-line
;;   ( s-single-next-line(n) | s-white* ) )?
(def s_single_next_line
  (let [cache (volatile! {})]
    (name* "s_single_next_line"
      (fn s_single_next_line-fn [parser n]
        (when DEBUG (debug-rule "s_single_next_line", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [s_flow_folded, n],
    (p/rep parser
       0,
      1,
      (p/all parser
         ns_single_char,
        nb_ns_single_in_line,
        (p/any parser
           [s_single_next_line, n],
          (p/rep parser
             0,
            nil,
            s_white
          )
        )
      ))
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [125]
;; nb-single-multi-line(n) ::=
;;   nb-ns-single-in-line
;;   ( s-single-next-line(n) | s-white* )
(def nb_single_multi_line
  (let [cache (volatile! {})]
    (name* "nb_single_multi_line"
      (fn nb_single_multi_line-fn [parser n]
        (when DEBUG (debug-rule "nb_single_multi_line", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     nb_ns_single_in_line,
    (p/any parser
       [s_single_next_line, n],
      (p/rep parser
         0,
        nil,
        s_white
      )
    )
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [126]
;; ns-plain-first(c) ::=
;;   ( ns-char - c-indicator )
;;   | ( ( '?' | ':' | '-' )
;;   <followed_by_an_ns-plain-safe(c)> )
(def ns_plain_first
  (let [cache (volatile! {})]
    (name* "ns_plain_first"
      (fn ns_plain_first-fn [parser c]
        (when DEBUG (debug-rule "ns_plain_first", c))
        (or (get @cache [c])
            (let [body
  (p/any parser
     (p/but parser
       ns_char,
      c_indicator
    ),
    (p/all parser
       (p/chars parser [0x2D 0x2D 0x3A 0x3A 0x3F 0x3F]),
      (p/chk parser
         "=",
        [ns_plain_safe, c]
      )
    )
  )]
              (vswap! cache assoc [c] body)
              body)))
      nil)))
;; [127]
;; ns-plain-safe(c) ::=
;;   ( c = flow-out => ns-plain-safe-out )
;;   ( c = flow-in => ns-plain-safe-in )
;;   ( c = block-key => ns-plain-safe-out )
;;   ( c = flow-key => ns-plain-safe-in )
(def ns_plain_safe
  (let [cache (volatile! {})]
    (name* "ns_plain_safe"
      (fn ns_plain_safe-fn [parser c]
        (when DEBUG (debug-rule "ns_plain_safe", c))
        (or (get @cache [c])
            (let [body
  (p/case-at parser
     c,
    {
      "block-key" ns_plain_safe_out,
      "flow-in" ns_plain_safe_in,
      "flow-key" ns_plain_safe_in,
      "flow-out" ns_plain_safe_out,
    }
  )]
              (vswap! cache assoc [c] body)
              body)))
      nil)))
;; [128]
;; ns-plain-safe-out ::=
;;   ns-char
(def ns_plain_safe_out
  (let [body (delay
               (let [parser nil]
    ns_char))]
    (name* "ns_plain_safe_out"
      (fn ns_plain_safe_out-fn [parser]
        (when DEBUG (debug-rule "ns_plain_safe_out"))
        @body)
      nil)))
;; [129]
;; ns-plain-safe-in ::=
;;   ns-char - c-flow-indicator
(def ns_plain_safe_in
  (let [body (delay
               (let [parser nil]
    (p/but parser
     ns_char,
    c_flow_indicator
  )))]
    (name* "ns_plain_safe_in"
      (fn ns_plain_safe_in-fn [parser]
        (when DEBUG (debug-rule "ns_plain_safe_in"))
        @body)
      nil)))
;; [130]
;; ns-plain-char(c) ::=
;;   ( ns-plain-safe(c) - ':' - '#' )
;;   | ( <an_ns-char_preceding> '#' )
;;   | ( ':' <followed_by_an_ns-plain-safe(c)> )
(def ns_plain_char
  (let [cache (volatile! {})]
    (name* "ns_plain_char"
      (fn ns_plain_char-fn [parser c]
        (when DEBUG (debug-rule "ns_plain_char", c))
        (or (get @cache [c])
            (let [body
  (p/any parser
     (p/but parser
       [ns_plain_safe, c],
      (p/chr parser
         ":"
      ),
      (p/chr parser
         "\u0023"
      )
    ),
    (p/all parser
       (p/chk parser
         "<=",
        ns_char
      ),
      (p/chr parser
         "\u0023"
      )
    ),
    (p/all parser
       (p/chr parser
         ":"
      ),
      (p/chk parser
         "=",
        [ns_plain_safe, c]
      )
    )
  )]
              (vswap! cache assoc [c] body)
              body)))
      nil)))
;; [131]
;; ns-plain(n,c) ::=
;;   ( c = flow-out => ns-plain-multi-line(n,c) )
;;   ( c = flow-in => ns-plain-multi-line(n,c) )
;;   ( c = block-key => ns-plain-one-line(c) )
;;   ( c = flow-key => ns-plain-one-line(c) )
(def ns_plain
  (let [cache (volatile! {})]
    (name* "ns_plain"
      (fn ns_plain-fn [parser n c]
        (when DEBUG (debug-rule "ns_plain", n, c))
        (or (get @cache [n c])
            (let [body
  (p/case-at parser
     c,
    {
      "block-key" [ns_plain_one_line, c],
      "flow-in" [ns_plain_multi_line, n, c],
      "flow-key" [ns_plain_one_line, c],
      "flow-out" [ns_plain_multi_line, n, c],
    }
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [132]
;; nb-ns-plain-in-line(c) ::=
;;   ( s-white*
;;   ns-plain-char(c) )*
(def nb_ns_plain_in_line
  (let [out-body (p/plain-in-line nil [0x21 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF] [0x21 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF])
        in-body (p/plain-in-line nil [0x21 0x2B 0x2D 0x5A 0x5C 0x5C 0x5E 0x7A 0x7C 0x7C 0x7E 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF] [0x21 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF])]
    (name* "nb_ns_plain_in_line"
      (fn nb_ns_plain_in_line-fn [parser c]
        (when DEBUG (debug-rule "nb_ns_plain_in_line", c))
        (case c
          ("flow-out" "block-key") out-body
          ("flow-in" "flow-key") in-body
          nil))
      nil)))

;; [133]
;; ns-plain-one-line(c) ::=
;;   ns-plain-first(c)
;;   nb-ns-plain-in-line(c)
(def ns_plain_one_line
  (let [cache (volatile! {})]
    (name* "ns_plain_one_line"
      (fn ns_plain_one_line-fn [parser c]
        (when DEBUG (debug-rule "ns_plain_one_line", c))
        (or (get @cache [c])
            (let [body
  (p/all parser
     [ns_plain_first, c],
    [nb_ns_plain_in_line, c]
  )]
              (vswap! cache assoc [c] body)
              body)))
      nil)))
;; [134]
;; s-ns-plain-next-line(n,c) ::=
;;   s-flow-folded(n)
;;   ns-plain-char(c) nb-ns-plain-in-line(c)
(def s_ns_plain_next_line
  (let [cache (volatile! {})]
    (name* "s_ns_plain_next_line"
      (fn s_ns_plain_next_line-fn [parser n c]
        (when DEBUG (debug-rule "s_ns_plain_next_line", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     [s_flow_folded, n],
    [ns_plain_char, c],
    [nb_ns_plain_in_line, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [135]
;; ns-plain-multi-line(n,c) ::=
;;   ns-plain-one-line(c)
;;   s-ns-plain-next-line(n,c)*
(def ns_plain_multi_line
  (let [cache (volatile! {})]
    (name* "ns_plain_multi_line"
      (fn ns_plain_multi_line-fn [parser n c]
        (when DEBUG (debug-rule "ns_plain_multi_line", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     [ns_plain_one_line, c],
    (p/rep parser
       0,
      nil,
      [s_ns_plain_next_line, n, c]
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [136]
;; in-flow(c) ::=
;;   ( c = flow-out => flow-in )
;;   ( c = flow-in => flow-in )
;;   ( c = block-key => flow-key )
;;   ( c = flow-key => flow-key )
(def in_flow
  (name* "in_flow"
    (fn in_flow-fn [parser c]
      (when DEBUG (debug-rule "in_flow", c))
    (p/flip parser
     c,
    {
      "block-key" "flow-key",
      "flow-in" "flow-in",
      "flow-key" "flow-key",
      "flow-out" "flow-in",
    }
  ))
    nil))

;; [137]
;; c-flow-sequence(n,c) ::=
;;   '[' s-separate(n,c)?
;;   ns-s-flow-seq-entries(n,in-flow(c))? ']'
(def c_flow_sequence
  (let [cache (volatile! {})]
    (name* "c_flow_sequence"
      (fn c_flow_sequence-fn [parser n c]
        (when DEBUG (debug-rule "c_flow_sequence", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     (p/chr parser
       "["
    ),
    (p/rep parser
       0,
      1,
      [s_separate, n, c]
    ),
    (p/rep2 parser
       0,
      1,
      [ns_s_flow_seq_entries, n, [in_flow, c]]
    ),
    (p/chr parser
       "]"
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [138]
;; ns-s-flow-seq-entries(n,c) ::=
;;   ns-flow-seq-entry(n,c)
;;   s-separate(n,c)?
;;   ( ',' s-separate(n,c)?
;;   ns-s-flow-seq-entries(n,c)? )?
(def ns_s_flow_seq_entries
  (let [cache (volatile! {})]
    (name* "ns_s_flow_seq_entries"
      (fn ns_s_flow_seq_entries-fn [parser n c]
        (when DEBUG (debug-rule "ns_s_flow_seq_entries", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     [ns_flow_seq_entry, n, c],
    (p/rep parser
       0,
      1,
      [s_separate, n, c]
    ),
    (p/rep2 parser
       0,
      1,
      (p/all parser
         (p/chr parser
           ","
        ),
        (p/rep parser
           0,
          1,
          [s_separate, n, c]
        ),
        (p/rep2 parser
           0,
          1,
          [ns_s_flow_seq_entries, n, c]
        )
      ))
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [139]
;; ns-flow-seq-entry(n,c) ::=
;;   ns-flow-pair(n,c) | ns-flow-node(n,c)
(def ns_flow_seq_entry
  (let [cache (volatile! {})]
    (name* "ns_flow_seq_entry"
      (fn ns_flow_seq_entry-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_seq_entry", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     [ns_flow_pair, n, c],
    [ns_flow_node, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [140]
;; c-flow-mapping(n,c) ::=
;;   '{' s-separate(n,c)?
;;   ns-s-flow-map-entries(n,in-flow(c))? '}'
(def c_flow_mapping
  (let [cache (volatile! {})]
    (name* "c_flow_mapping"
      (fn c_flow_mapping-fn [parser n c]
        (when DEBUG (debug-rule "c_flow_mapping", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     (p/chr parser
       "\u007B"
    ),
    (p/rep parser
       0,
      1,
      [s_separate, n, c]
    ),
    (p/rep2 parser
       0,
      1,
      [ns_s_flow_map_entries, n, [in_flow, c]]
    ),
    (p/chr parser
       "\u007D"
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [141]
;; ns-s-flow-map-entries(n,c) ::=
;;   ns-flow-map-entry(n,c)
;;   s-separate(n,c)?
;;   ( ',' s-separate(n,c)?
;;   ns-s-flow-map-entries(n,c)? )?
(def ns_s_flow_map_entries
  (let [cache (volatile! {})]
    (name* "ns_s_flow_map_entries"
      (fn ns_s_flow_map_entries-fn [parser n c]
        (when DEBUG (debug-rule "ns_s_flow_map_entries", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     [ns_flow_map_entry, n, c],
    (p/rep parser
       0,
      1,
      [s_separate, n, c]
    ),
    (p/rep2 parser
       0,
      1,
      (p/all parser
         (p/chr parser
           ","
        ),
        (p/rep parser
           0,
          1,
          [s_separate, n, c]
        ),
        (p/rep2 parser
           0,
          1,
          [ns_s_flow_map_entries, n, c]
        )
      ))
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [142]
;; ns-flow-map-entry(n,c) ::=
;;   ( '?' s-separate(n,c)
;;   ns-flow-map-explicit-entry(n,c) )
;;   | ns-flow-map-implicit-entry(n,c)
(def ns_flow_map_entry
  (let [cache (volatile! {})]
    (name* "ns_flow_map_entry"
      (fn ns_flow_map_entry-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_map_entry", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     (p/all parser
       (p/chr parser
         "?"
      ),
      (p/chk parser
         "=",
        (p/any parser
           (p/end-of-stream parser),
          s_white,
          b_break
        )
      ),
      [s_separate, n, c],
      [ns_flow_map_explicit_entry, n, c]
    ),
    [ns_flow_map_implicit_entry, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [143]
;; ns-flow-map-explicit-entry(n,c) ::=
;;   ns-flow-map-implicit-entry(n,c)
;;   | ( e-node
;;   e-node )
(def ns_flow_map_explicit_entry
  (let [cache (volatile! {})]
    (name* "ns_flow_map_explicit_entry"
      (fn ns_flow_map_explicit_entry-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_map_explicit_entry", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     [ns_flow_map_implicit_entry, n, c],
    (p/all parser
       e_node,
      e_node
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [144]
;; ns-flow-map-implicit-entry(n,c) ::=
;;   ns-flow-map-yaml-key-entry(n,c)
;;   | c-ns-flow-map-empty-key-entry(n,c)
;;   | c-ns-flow-map-json-key-entry(n,c)
(def ns_flow_map_implicit_entry
  (let [cache (volatile! {})]
    (name* "ns_flow_map_implicit_entry"
      (fn ns_flow_map_implicit_entry-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_map_implicit_entry", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     [ns_flow_map_yaml_key_entry, n, c],
    [c_ns_flow_map_empty_key_entry, n, c],
    [c_ns_flow_map_json_key_entry, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [145]
;; ns-flow-map-yaml-key-entry(n,c) ::=
;;   ns-flow-yaml-node(n,c)
;;   ( ( s-separate(n,c)?
;;   c-ns-flow-map-separate-value(n,c) )
;;   | e-node )
(def ns_flow_map_yaml_key_entry
  (let [cache (volatile! {})]
    (name* "ns_flow_map_yaml_key_entry"
      (fn ns_flow_map_yaml_key_entry-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_map_yaml_key_entry", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     [ns_flow_yaml_node, n, c],
    (p/any parser
       (p/all parser
         (p/rep parser
           0,
          1,
          [s_separate, n, c]
        ),
        [c_ns_flow_map_separate_value, n, c]
      ),
      e_node
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [146]
;; c-ns-flow-map-empty-key-entry(n,c) ::=
;;   e-node
;;   c-ns-flow-map-separate-value(n,c)
(def c_ns_flow_map_empty_key_entry
  (let [cache (volatile! {})]
    (name* "c_ns_flow_map_empty_key_entry"
      (fn c_ns_flow_map_empty_key_entry-fn [parser n c]
        (when DEBUG (debug-rule "c_ns_flow_map_empty_key_entry", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     e_node,
    [c_ns_flow_map_separate_value, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [147]
;; c-ns-flow-map-separate-value(n,c) ::=
;;   ':' <not_followed_by_an_ns-plain-safe(c)>
;;   ( ( s-separate(n,c) ns-flow-node(n,c) )
;;   | e-node )
(def c_ns_flow_map_separate_value
  (let [cache (volatile! {})]
    (name* "c_ns_flow_map_separate_value"
      (fn c_ns_flow_map_separate_value-fn [parser n c]
        (when DEBUG (debug-rule "c_ns_flow_map_separate_value", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     (p/chr parser
       ":"
    ),
    (p/chk parser
       "!",
      [ns_plain_safe, c]
    ),
    (p/any parser
       (p/all parser
         [s_separate, n, c],
        [ns_flow_node, n, c]
      ),
      e_node
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [148]
;; c-ns-flow-map-json-key-entry(n,c) ::=
;;   c-flow-json-node(n,c)
;;   ( ( s-separate(n,c)?
;;   c-ns-flow-map-adjacent-value(n,c) )
;;   | e-node )
(def c_ns_flow_map_json_key_entry
  (let [cache (volatile! {})]
    (name* "c_ns_flow_map_json_key_entry"
      (fn c_ns_flow_map_json_key_entry-fn [parser n c]
        (when DEBUG (debug-rule "c_ns_flow_map_json_key_entry", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     [c_flow_json_node, n, c],
    (p/any parser
       (p/all parser
         (p/rep parser
           0,
          1,
          [s_separate, n, c]
        ),
        [c_ns_flow_map_adjacent_value, n, c]
      ),
      e_node
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [149]
;; c-ns-flow-map-adjacent-value(n,c) ::=
;;   ':' ( (
;;   s-separate(n,c)?
;;   ns-flow-node(n,c) )
;;   | e-node )
(def c_ns_flow_map_adjacent_value
  (let [cache (volatile! {})]
    (name* "c_ns_flow_map_adjacent_value"
      (fn c_ns_flow_map_adjacent_value-fn [parser n c]
        (when DEBUG (debug-rule "c_ns_flow_map_adjacent_value", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     (p/chr parser
       ":"
    ),
    (p/any parser
       (p/all parser
         (p/rep parser
           0,
          1,
          [s_separate, n, c]
        ),
        [ns_flow_node, n, c]
      ),
      e_node
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [150]
;; ns-flow-pair(n,c) ::=
;;   ( '?' s-separate(n,c)
;;   ns-flow-map-explicit-entry(n,c) )
;;   | ns-flow-pair-entry(n,c)
(def ns_flow_pair
  (let [cache (volatile! {})]
    (name* "ns_flow_pair"
      (fn ns_flow_pair-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_pair", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     (p/all parser
       (p/chr parser
         "?"
      ),
      (p/chk parser
         "=",
        (p/any parser
           (p/end-of-stream parser),
          s_white,
          b_break
        )
      ),
      [s_separate, n, c],
      [ns_flow_map_explicit_entry, n, c]
    ),
    [ns_flow_pair_entry, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [151]
;; ns-flow-pair-entry(n,c) ::=
;;   ns-flow-pair-yaml-key-entry(n,c)
;;   | c-ns-flow-map-empty-key-entry(n,c)
;;   | c-ns-flow-pair-json-key-entry(n,c)
(def ns_flow_pair_entry
  (let [cache (volatile! {})]
    (name* "ns_flow_pair_entry"
      (fn ns_flow_pair_entry-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_pair_entry", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     [ns_flow_pair_yaml_key_entry, n, c],
    [c_ns_flow_map_empty_key_entry, n, c],
    [c_ns_flow_pair_json_key_entry, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [152]
;; ns-flow-pair-yaml-key-entry(n,c) ::=
;;   ns-s-implicit-yaml-key(flow-key)
;;   c-ns-flow-map-separate-value(n,c)
(def ns_flow_pair_yaml_key_entry
  (let [cache (volatile! {})]
    (name* "ns_flow_pair_yaml_key_entry"
      (fn ns_flow_pair_yaml_key_entry-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_pair_yaml_key_entry", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     [ns_s_implicit_yaml_key, "flow-key"],
    [c_ns_flow_map_separate_value, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [153]
;; c-ns-flow-pair-json-key-entry(n,c) ::=
;;   c-s-implicit-json-key(flow-key)
;;   c-ns-flow-map-adjacent-value(n,c)
(def c_ns_flow_pair_json_key_entry
  (let [cache (volatile! {})]
    (name* "c_ns_flow_pair_json_key_entry"
      (fn c_ns_flow_pair_json_key_entry-fn [parser n c]
        (when DEBUG (debug-rule "c_ns_flow_pair_json_key_entry", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     [c_s_implicit_json_key, "flow-key"],
    [c_ns_flow_map_adjacent_value, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [154]
;; ns-s-implicit-yaml-key(c) ::=
;;   ns-flow-yaml-node(n/a,c)
;;   s-separate-in-line?
;;   <at_most_1024_characters_altogether>
(def ns_s_implicit_yaml_key
  (let [cache (volatile! {})]
    (name* "ns_s_implicit_yaml_key"
      (fn ns_s_implicit_yaml_key-fn [parser c]
        (when DEBUG (debug-rule "ns_s_implicit_yaml_key", c))
        (or (get @cache [c])
            (let [body
  (p/all parser
     [ns_flow_yaml_node, nil, c],
    (p/rep parser
       0,
      1,
      s_separate_in_line
    )
  )]
              (vswap! cache assoc [c] body)
              body)))
      nil)))
;; [155]
;; c-s-implicit-json-key(c) ::=
;;   c-flow-json-node(n/a,c)
;;   s-separate-in-line?
;;   <at_most_1024_characters_altogether>
(def c_s_implicit_json_key
  (let [cache (volatile! {})]
    (name* "c_s_implicit_json_key"
      (fn c_s_implicit_json_key-fn [parser c]
        (when DEBUG (debug-rule "c_s_implicit_json_key", c))
        (or (get @cache [c])
            (let [body
  (p/all parser
     [c_flow_json_node, nil, c],
    (p/rep parser
       0,
      1,
      s_separate_in_line
    )
  )]
              (vswap! cache assoc [c] body)
              body)))
      nil)))
;; [156]
;; ns-flow-yaml-content(n,c) ::=
;;   ns-plain(n,c)
(def ns_flow_yaml_content
  (let [cache (volatile! {})]
    (name* "ns_flow_yaml_content"
      (fn ns_flow_yaml_content-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_yaml_content", n, c))
        (or (get @cache [n c])
            (let [body
  [ns_plain, n, c]]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [157]
;; c-flow-json-content(n,c) ::=
;;   c-flow-sequence(n,c) | c-flow-mapping(n,c)
;;   | c-single-quoted(n,c) | c-double-quoted(n,c)
(def c_flow_json_content
  (let [cache (volatile! {})]
    (name* "c_flow_json_content"
      (fn c_flow_json_content-fn [parser n c]
        (when DEBUG (debug-rule "c_flow_json_content", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     [c_flow_sequence, n, c],
    [c_flow_mapping, n, c],
    [c_single_quoted, n, c],
    [c_double_quoted, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [158]
;; ns-flow-content(n,c) ::=
;;   ns-flow-yaml-content(n,c) | c-flow-json-content(n,c)
(def ns_flow_content
  (let [cache (volatile! {})]
    (name* "ns_flow_content"
      (fn ns_flow_content-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_content", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     [ns_flow_yaml_content, n, c],
    [c_flow_json_content, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [159]
;; ns-flow-yaml-node(n,c) ::=
;;   c-ns-alias-node
;;   | ns-flow-yaml-content(n,c)
;;   | ( c-ns-properties(n,c)
;;   ( ( s-separate(n,c)
;;   ns-flow-yaml-content(n,c) )
;;   | e-scalar ) )
(def ns_flow_yaml_node
  (let [cache (volatile! {})]
    (name* "ns_flow_yaml_node"
      (fn ns_flow_yaml_node-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_yaml_node", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     c_ns_alias_node,
    [ns_flow_yaml_content, n, c],
    (p/all parser
       [c_ns_properties, n, c],
      (p/any parser
         (p/all parser
           [s_separate, n, c],
          [ns_flow_content, n, c]
        ),
        e_scalar
      )
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [160]
;; c-flow-json-node(n,c) ::=
;;   ( c-ns-properties(n,c)
;;   s-separate(n,c) )?
;;   c-flow-json-content(n,c)
(def c_flow_json_node
  (let [cache (volatile! {})]
    (name* "c_flow_json_node"
      (fn c_flow_json_node-fn [parser n c]
        (when DEBUG (debug-rule "c_flow_json_node", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     (p/rep parser
       0,
      1,
      (p/all parser
         [c_ns_properties, n, c],
        [s_separate, n, c]
      )),
    [c_flow_json_content, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [161]
;; ns-flow-node(n,c) ::=
;;   c-ns-alias-node
;;   | ns-flow-content(n,c)
;;   | ( c-ns-properties(n,c)
;;   ( ( s-separate(n,c)
;;   ns-flow-content(n,c) )
;;   | e-scalar ) )
(def ns_flow_node
  (let [cache (volatile! {})]
    (name* "ns_flow_node"
      (fn ns_flow_node-fn [parser n c]
        (when DEBUG (debug-rule "ns_flow_node", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     c_ns_alias_node,
    [ns_flow_content, n, c],
    (p/all parser
       [c_ns_properties, n, c],
      (p/any parser
         (p/all parser
           [s_separate, n, c],
          [ns_flow_content, n, c]
        ),
        e_scalar
      )
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [162]
;; c-b-block-header(m,t) ::=
;;   ( ( c-indentation-indicator(m)
;;   c-chomping-indicator(t) )
;;   | ( c-chomping-indicator(t)
;;   c-indentation-indicator(m) ) )
;;   s-b-comment
(def c_b_block_header
  (let [cache (volatile! {})]
    (name* "c_b_block_header"
      (fn c_b_block_header-fn [parser n]
        (when DEBUG (debug-rule "c_b_block_header", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/any parser
       (p/all parser
         [c_indentation_indicator, n],
        c_chomping_indicator,
        (p/chk parser
           "=",
          (p/any parser
             (p/end-of-stream parser),
            s_white,
            b_break
          )
        )
      ),
      (p/all parser
         c_chomping_indicator,
        [c_indentation_indicator, n],
        (p/chk parser
           "=",
          (p/any parser
             (p/end-of-stream parser),
            s_white,
            b_break
          )
        )
      )
    ),
    s_b_comment
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [163]
;; c-indentation-indicator(m) ::=
;;   ( ns-dec-digit => m = ns-dec-digit - x:30 )
;;   ( <empty> => m = auto-detect() )
(def c_indentation_indicator
  (let [cache (volatile! {})]
    (name* "c_indentation_indicator"
      (fn c_indentation_indicator-fn [parser n]
        (when DEBUG (debug-rule "c_indentation_indicator", n))
        (or (get @cache [n])
            (let [body
  (p/any parser
     (p/if* parser
       (p/rng parser
         "\u0031",
        "\u0039"
      ),
      (p/set* parser
         "m",
        (p/ord parser
           (p/match parser)
        )
      )
    ),
    (p/if* parser
       empty,
      (p/set* parser
         "m",
        [auto_detect, n]
      )
    )
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [164]
;; c-chomping-indicator(t) ::=
;;   ( '-' => t = strip )
;;   ( '+' => t = keep )
;;   ( <empty> => t = clip )
(def c_chomping_indicator
  (let [body (delay
               (let [parser nil]
    (p/any parser
     (p/if* parser
       (p/chr parser
         "-"
      ),
      (p/set* parser
         "t",
        "strip"
      )
    ),
    (p/if* parser
       (p/chr parser
         "+"
      ),
      (p/set* parser
         "t",
        "keep"
      )
    ),
    (p/if* parser
       empty,
      (p/set* parser
         "t",
        "clip"
      )
    )
  )))]
    (name* "c_chomping_indicator"
      (fn c_chomping_indicator-fn [parser]
        (when DEBUG (debug-rule "c_chomping_indicator"))
        @body)
      nil)))
;; [165]
;; b-chomped-last(t) ::=
;;   ( t = strip => b-non-content | <end_of_file> )
;;   ( t = clip => b-as-line-feed | <end_of_file> )
;;   ( t = keep => b-as-line-feed | <end_of_file> )
(def b_chomped_last
  (let [cache (volatile! {})]
    (name* "b_chomped_last"
      (fn b_chomped_last-fn [parser t]
        (when DEBUG (debug-rule "b_chomped_last", t))
        (or (get @cache [t])
            (let [body
  (p/case-at parser
     t,
    {
      "clip" (p/any parser b_as_line_feed, (p/end-of-stream parser) ),
      "keep" (p/any parser b_as_line_feed, (p/end-of-stream parser) ),
      "strip" (p/any parser b_non_content, (p/end-of-stream parser) ),
    }
  )]
              (vswap! cache assoc [t] body)
              body)))
      nil)))
;; [166]
;; l-chomped-empty(n,t) ::=
;;   ( t = strip => l-strip-empty(n) )
;;   ( t = clip => l-strip-empty(n) )
;;   ( t = keep => l-keep-empty(n) )
(def l_chomped_empty
  (let [cache (volatile! {})]
    (name* "l_chomped_empty"
      (fn l_chomped_empty-fn [parser n t]
        (when DEBUG (debug-rule "l_chomped_empty", n, t))
        (or (get @cache [n t])
            (let [body
  (p/case-at parser
     t,
    {
      "clip" [l_strip_empty, n],
      "keep" [l_keep_empty, n],
      "strip" [l_strip_empty, n],
    }
  )]
              (vswap! cache assoc [n t] body)
              body)))
      nil)))
;; [167]
;; l-strip-empty(n) ::=
;;   ( s-indent(<=n) b-non-content )*
;;   l-trail-comments(n)?
(def l_strip_empty
  (let [cache (volatile! {})]
    (name* "l_strip_empty"
      (fn l_strip_empty-fn [parser n]
        (when DEBUG (debug-rule "l_strip_empty", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/rep parser
       0,
      nil,
      (p/all parser
         [s_indent_le, n],
        b_non_content
      )),
    (p/rep2 parser
       0,
      1,
      [l_trail_comments, n]
    )
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [168]
;; l-keep-empty(n) ::=
;;   l-empty(n,block-in)*
;;   l-trail-comments(n)?
(def l_keep_empty
  (let [cache (volatile! {})]
    (name* "l_keep_empty"
      (fn l_keep_empty-fn [parser n]
        (when DEBUG (debug-rule "l_keep_empty", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/rep parser
       0,
      nil,
      [l_empty, n, "block-in"]
    ),
    (p/rep2 parser
       0,
      1,
      [l_trail_comments, n]
    )
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [169]
;; l-trail-comments(n) ::=
;;   s-indent(<n)
;;   c-nb-comment-text b-comment
;;   l-comment*
(def l_trail_comments
  (let [cache (volatile! {})]
    (name* "l_trail_comments"
      (fn l_trail_comments-fn [parser n]
        (when DEBUG (debug-rule "l_trail_comments", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [s_indent_lt, n],
    c_nb_comment_text,
    b_comment,
    (p/rep parser
       0,
      nil,
      l_comment
    )
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [170]
;; c-l+literal(n) ::=
;;   '|' c-b-block-header(m,t)
;;   l-literal-content(n+m,t)
(def c_l_literal
  (let [cache (volatile! {})]
    (name* "c_l_literal"
      (fn c_l_literal-fn [parser n]
        (when DEBUG (debug-rule "c_l_literal", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/chr parser
       "|"
    ),
    [c_b_block_header, n],
    [l_literal_content, (p/add parser
       n,
      (p/m parser
      )
    ), (p/t parser
    )]
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [171]
;; l-nb-literal-text(n) ::=
;;   l-empty(n,block-in)*
;;   s-indent(n) nb-char+
(def l_nb_literal_text
  (let [cache (volatile! {})]
    (name* "l_nb_literal_text"
      (fn l_nb_literal_text-fn [parser n]
        (when DEBUG (debug-rule "l_nb_literal_text", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/rep parser
       0,
      nil,
      [l_empty, n, "block-in"]
    ),
    [s_indent, n],
    (p/chars-rep parser 1, nil,
           [0x9 0x9 0x20 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF] "rep2")
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [172]
;; b-nb-literal-next(n) ::=
;;   b-as-line-feed
;;   l-nb-literal-text(n)
(def b_nb_literal_next
  (let [cache (volatile! {})]
    (name* "b_nb_literal_next"
      (fn b_nb_literal_next-fn [parser n]
        (when DEBUG (debug-rule "b_nb_literal_next", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     b_as_line_feed,
    [l_nb_literal_text, n]
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [173]
;; l-literal-content(n,t) ::=
;;   ( l-nb-literal-text(n)
;;   b-nb-literal-next(n)*
;;   b-chomped-last(t) )?
;;   l-chomped-empty(n,t)
(def l_literal_content
  (let [cache (volatile! {})]
    (name* "l_literal_content"
      (fn l_literal_content-fn [parser n t]
        (when DEBUG (debug-rule "l_literal_content", n, t))
        (or (get @cache [n t])
            (let [body
  (p/all parser
     (p/rep parser
       0,
      1,
      (p/all parser
         [l_nb_literal_text, n],
        (p/rep parser
           0,
          nil,
          [b_nb_literal_next, n]
        ),
        [b_chomped_last, t]
      )),
    [l_chomped_empty, n, t]
  )]
              (vswap! cache assoc [n t] body)
              body)))
      nil)))
;; [174]
;; c-l+folded(n) ::=
;;   '>' c-b-block-header(m,t)
;;   l-folded-content(n+m,t)
(def c_l_folded
  (let [cache (volatile! {})]
    (name* "c_l_folded"
      (fn c_l_folded-fn [parser n]
        (when DEBUG (debug-rule "c_l_folded", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/chr parser
       ">"
    ),
    [c_b_block_header, n],
    [l_folded_content, (p/add parser
       n,
      (p/m parser
      )
    ), (p/t parser
    )]
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [175]
;; s-nb-folded-text(n) ::=
;;   s-indent(n) ns-char
;;   nb-char*
(def s_nb_folded_text
  (let [cache (volatile! {})]
    (name* "s_nb_folded_text"
      (fn s_nb_folded_text-fn [parser n]
        (when DEBUG (debug-rule "s_nb_folded_text", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [s_indent, n],
    ns_char,
    (p/chars-rep parser 0, nil,
           [0x9 0x9 0x20 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF] "rep")
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [176]
;; l-nb-folded-lines(n) ::=
;;   s-nb-folded-text(n)
;;   ( b-l-folded(n,block-in) s-nb-folded-text(n) )*
(def l_nb_folded_lines
  (let [cache (volatile! {})]
    (name* "l_nb_folded_lines"
      (fn l_nb_folded_lines-fn [parser n]
        (when DEBUG (debug-rule "l_nb_folded_lines", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [s_nb_folded_text, n],
    (p/rep parser
       0,
      nil,
      (p/all parser
         [b_l_folded, n, "block-in"],
        [s_nb_folded_text, n]
      ))
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [177]
;; s-nb-spaced-text(n) ::=
;;   s-indent(n) s-white
;;   nb-char*
(def s_nb_spaced_text
  (let [cache (volatile! {})]
    (name* "s_nb_spaced_text"
      (fn s_nb_spaced_text-fn [parser n]
        (when DEBUG (debug-rule "s_nb_spaced_text", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [s_indent, n],
    s_white,
    (p/chars-rep parser 0, nil,
           [0x9 0x9 0x20 0x7E 0x85 0x85 0xA0 0xD7FF 0xE000 0xFEFE 0xFF00 0xFFFD 0x10000 0x10FFFF] "rep")
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [178]
;; b-l-spaced(n) ::=
;;   b-as-line-feed
;;   l-empty(n,block-in)*
(def b_l_spaced
  (let [cache (volatile! {})]
    (name* "b_l_spaced"
      (fn b_l_spaced-fn [parser n]
        (when DEBUG (debug-rule "b_l_spaced", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     b_as_line_feed,
    (p/rep parser
       0,
      nil,
      [l_empty, n, "block-in"]
    )
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [179]
;; l-nb-spaced-lines(n) ::=
;;   s-nb-spaced-text(n)
;;   ( b-l-spaced(n) s-nb-spaced-text(n) )*
(def l_nb_spaced_lines
  (let [cache (volatile! {})]
    (name* "l_nb_spaced_lines"
      (fn l_nb_spaced_lines-fn [parser n]
        (when DEBUG (debug-rule "l_nb_spaced_lines", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [s_nb_spaced_text, n],
    (p/rep parser
       0,
      nil,
      (p/all parser
         [b_l_spaced, n],
        [s_nb_spaced_text, n]
      ))
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [180]
;; l-nb-same-lines(n) ::=
;;   l-empty(n,block-in)*
;;   ( l-nb-folded-lines(n) | l-nb-spaced-lines(n) )
(def l_nb_same_lines
  (let [cache (volatile! {})]
    (name* "l_nb_same_lines"
      (fn l_nb_same_lines-fn [parser n]
        (when DEBUG (debug-rule "l_nb_same_lines", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/rep parser
       0,
      nil,
      [l_empty, n, "block-in"]
    ),
    (p/any parser
       [l_nb_folded_lines, n],
      [l_nb_spaced_lines, n]
    )
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [181]
;; l-nb-diff-lines(n) ::=
;;   l-nb-same-lines(n)
;;   ( b-as-line-feed l-nb-same-lines(n) )*
(def l_nb_diff_lines
  (let [cache (volatile! {})]
    (name* "l_nb_diff_lines"
      (fn l_nb_diff_lines-fn [parser n]
        (when DEBUG (debug-rule "l_nb_diff_lines", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [l_nb_same_lines, n],
    (p/rep parser
       0,
      nil,
      (p/all parser
         b_as_line_feed,
        [l_nb_same_lines, n]
      ))
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [182]
;; l-folded-content(n,t) ::=
;;   ( l-nb-diff-lines(n)
;;   b-chomped-last(t) )?
;;   l-chomped-empty(n,t)
(def l_folded_content
  (let [cache (volatile! {})]
    (name* "l_folded_content"
      (fn l_folded_content-fn [parser n t]
        (when DEBUG (debug-rule "l_folded_content", n, t))
        (or (get @cache [n t])
            (let [body
  (p/all parser
     (p/rep parser
       0,
      1,
      (p/all parser
         [l_nb_diff_lines, n],
        [b_chomped_last, t]
      )),
    [l_chomped_empty, n, t]
  )]
              (vswap! cache assoc [n t] body)
              body)))
      nil)))
;; [183]
;; l+block-sequence(n) ::=
;;   ( s-indent(n+m)
;;   c-l-block-seq-entry(n+m) )+
;;   <for_some_fixed_auto-detected_m_>_0>
(def l_block_sequence
  (name* "l_block_sequence"
    (fn l_block_sequence-fn [parser n]
  ;; m must be > 0 (JS treats 0 as falsy, Clojure doesn't)
  (let [m (p/call parser [auto_detect_indent, n] "number")]
    (if (and m (pos? m))
    (do
      (when DEBUG (debug-rule "l_block_sequence", n))
    (p/all parser
     (p/rep parser
       1,
      nil,
      (p/all parser
         [s_indent, (+ n m)],
        [c_l_block_seq_entry, (+ n m)]
      ))
  ))
      false)))
    nil))

;; [184]
;; c-l-block-seq-entry(n) ::=
;;   '-' <not_followed_by_an_ns-char>
;;   s-l+block-indented(n,block-in)
(def c_l_block_seq_entry
  (let [cache (volatile! {})]
    (name* "c_l_block_seq_entry"
      (fn c_l_block_seq_entry-fn [parser n]
        (when DEBUG (debug-rule "c_l_block_seq_entry", n))
        (if (p/ahead? parser [0x2D 0x2D])
          (or (get @cache [n])
              (let [body
  (p/all parser
     (p/chr parser
       "-"
    ),
    (p/chk parser
       "!",
      ns_char
    ),
    [s_l_block_indented, n, "block-in"]
  )]
                (vswap! cache assoc [n] body)
                body))
          false))
      nil)))
;; [185]
;; s-l+block-indented(n,c) ::=
;;   ( s-indent(m)
;;   ( ns-l-compact-sequence(n+1+m)
;;   | ns-l-compact-mapping(n+1+m) ) )
;;   | s-l+block-node(n,c)
;;   | ( e-node s-l-comments )
(def s_l_block_indented
  (name* "s_l_block_indented"
    (fn s_l_block_indented-fn [parser n c]
  (let [m (p/call parser [auto_detect_indent, n] "number")]
      (when DEBUG (debug-rule "s_l_block_indented", n, c))
    (p/any parser
     (p/all parser
       [s_indent, m],
      (p/any parser
         [ns_l_compact_sequence, (+ n (+ 1 m))],
        [ns_l_compact_mapping, (+ n (+ 1 m))]
      )
    ),
    [s_l_block_node, n, c],
    (p/all parser
       e_node,
      s_l_comments
    )
  )))
    nil))

;; [186]
;; ns-l-compact-sequence(n) ::=
;;   c-l-block-seq-entry(n)
;;   ( s-indent(n) c-l-block-seq-entry(n) )*
(def ns_l_compact_sequence
  (let [cache (volatile! {})]
    (name* "ns_l_compact_sequence"
      (fn ns_l_compact_sequence-fn [parser n]
        (when DEBUG (debug-rule "ns_l_compact_sequence", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [c_l_block_seq_entry, n],
    (p/rep parser
       0,
      nil,
      (p/all parser
         [s_indent, n],
        [c_l_block_seq_entry, n]
      ))
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [187]
;; l+block-mapping(n) ::=
;;   ( s-indent(n+m)
;;   ns-l-block-map-entry(n+m) )+
;;   <for_some_fixed_auto-detected_m_>_0>
(def l_block_mapping
  (name* "l_block_mapping"
    (fn l_block_mapping-fn [parser n]
  ;; m must be > 0 (JS treats 0 as falsy, Clojure doesn't)
  (let [m (p/call parser [auto_detect_indent, n] "number")]
    (if (and m (pos? m))
    (do
      (when DEBUG (debug-rule "l_block_mapping", n))
    (p/all parser
     (p/rep parser
       1,
      nil,
      (p/all parser
         [s_indent, (+ n m)],
        [ns_l_block_map_entry, (+ n m)]
      ))
  ))
      false)))
    nil))

;; [188]
;; ns-l-block-map-entry(n) ::=
;;   c-l-block-map-explicit-entry(n)
;;   | ns-l-block-map-implicit-entry(n)
(def ns_l_block_map_entry
  (let [cache (volatile! {})]
    (name* "ns_l_block_map_entry"
      (fn ns_l_block_map_entry-fn [parser n]
        (when DEBUG (debug-rule "ns_l_block_map_entry", n))
        (or (get @cache [n])
            (let [body
  (p/any parser
     [c_l_block_map_explicit_entry, n],
    [ns_l_block_map_implicit_entry, n]
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [189]
;; c-l-block-map-explicit-entry(n) ::=
;;   c-l-block-map-explicit-key(n)
;;   ( l-block-map-explicit-value(n)
;;   | e-node )
(def c_l_block_map_explicit_entry
  (let [cache (volatile! {})]
    (name* "c_l_block_map_explicit_entry"
      (fn c_l_block_map_explicit_entry-fn [parser n]
        (when DEBUG (debug-rule "c_l_block_map_explicit_entry", n))
        (if (p/ahead? parser [0x3F 0x3F])
          (or (get @cache [n])
              (let [body
  (p/all parser
     [c_l_block_map_explicit_key, n],
    (p/any parser
       [l_block_map_explicit_value, n],
      e_node
    )
  )]
                (vswap! cache assoc [n] body)
                body))
          false))
      nil)))
;; [190]
;; c-l-block-map-explicit-key(n) ::=
;;   '?'
;;   s-l+block-indented(n,block-out)
(def c_l_block_map_explicit_key
  (let [cache (volatile! {})]
    (name* "c_l_block_map_explicit_key"
      (fn c_l_block_map_explicit_key-fn [parser n]
        (when DEBUG (debug-rule "c_l_block_map_explicit_key", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/chr parser
       "?"
    ),
    (p/chk parser
       "=",
      (p/any parser
         (p/end-of-stream parser),
        s_white,
        b_break
      )
    ),
    [s_l_block_indented, n, "block-out"]
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [191]
;; l-block-map-explicit-value(n) ::=
;;   s-indent(n)
;;   ':' s-l+block-indented(n,block-out)
(def l_block_map_explicit_value
  (let [cache (volatile! {})]
    (name* "l_block_map_explicit_value"
      (fn l_block_map_explicit_value-fn [parser n]
        (when DEBUG (debug-rule "l_block_map_explicit_value", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [s_indent, n],
    (p/chr parser
       ":"
    ),
    [s_l_block_indented, n, "block-out"]
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [192]
;; ns-l-block-map-implicit-entry(n) ::=
;;   (
;;   ns-s-block-map-implicit-key
;;   | e-node )
;;   c-l-block-map-implicit-value(n)
(def ns_l_block_map_implicit_entry
  (let [cache (volatile! {})]
    (name* "ns_l_block_map_implicit_entry"
      (fn ns_l_block_map_implicit_entry-fn [parser n]
        (when DEBUG (debug-rule "ns_l_block_map_implicit_entry", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/any parser
       ns_s_block_map_implicit_key,
      e_node
    ),
    [c_l_block_map_implicit_value, n]
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [193]
;; ns-s-block-map-implicit-key ::=
;;   c-s-implicit-json-key(block-key)
;;   | ns-s-implicit-yaml-key(block-key)
(def ns_s_block_map_implicit_key
  (let [body (delay
               (let [parser nil]
    (p/any parser
     [c_s_implicit_json_key, "block-key"],
    [ns_s_implicit_yaml_key, "block-key"]
  )))]
    (name* "ns_s_block_map_implicit_key"
      (fn ns_s_block_map_implicit_key-fn [parser]
        (when DEBUG (debug-rule "ns_s_block_map_implicit_key"))
        @body)
      nil)))
;; [194]
;; c-l-block-map-implicit-value(n) ::=
;;   ':' (
;;   s-l+block-node(n,block-out)
;;   | ( e-node s-l-comments ) )
(def c_l_block_map_implicit_value
  (let [cache (volatile! {})]
    (name* "c_l_block_map_implicit_value"
      (fn c_l_block_map_implicit_value-fn [parser n]
        (when DEBUG (debug-rule "c_l_block_map_implicit_value", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     (p/chr parser
       ":"
    ),
    (p/any parser
       [s_l_block_node, n, "block-out"],
      (p/all parser
         e_node,
        s_l_comments
      )
    )
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [195]
;; ns-l-compact-mapping(n) ::=
;;   ns-l-block-map-entry(n)
;;   ( s-indent(n) ns-l-block-map-entry(n) )*
(def ns_l_compact_mapping
  (let [cache (volatile! {})]
    (name* "ns_l_compact_mapping"
      (fn ns_l_compact_mapping-fn [parser n]
        (when DEBUG (debug-rule "ns_l_compact_mapping", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [ns_l_block_map_entry, n],
    (p/rep parser
       0,
      nil,
      (p/all parser
         [s_indent, n],
        [ns_l_block_map_entry, n]
      ))
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [196]
;; s-l+block-node(n,c) ::=
;;   s-l+block-in-block(n,c) | s-l+flow-in-block(n)
(def s_l_block_node
  (let [cache (volatile! {})]
    (name* "s_l_block_node"
      (fn s_l_block_node-fn [parser n c]
        (when DEBUG (debug-rule "s_l_block_node", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     [s_l_block_in_block, n, c],
    [s_l_flow_in_block, n]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [197]
;; s-l+flow-in-block(n) ::=
;;   s-separate(n+1,flow-out)
;;   ns-flow-node(n+1,flow-out) s-l-comments
(def s_l_flow_in_block
  (let [cache (volatile! {})]
    (name* "s_l_flow_in_block"
      (fn s_l_flow_in_block-fn [parser n]
        (when DEBUG (debug-rule "s_l_flow_in_block", n))
        (or (get @cache [n])
            (let [body
  (p/all parser
     [s_separate, (+ n 1), "flow-out"],
    [ns_flow_node, (+ n 1), "flow-out"],
    s_l_comments
  )]
              (vswap! cache assoc [n] body)
              body)))
      nil)))
;; [198]
;; s-l+block-in-block(n,c) ::=
;;   s-l+block-scalar(n,c) | s-l+block-collection(n,c)
(def s_l_block_in_block
  (let [cache (volatile! {})]
    (name* "s_l_block_in_block"
      (fn s_l_block_in_block-fn [parser n c]
        (when DEBUG (debug-rule "s_l_block_in_block", n, c))
        (or (get @cache [n c])
            (let [body
  (p/any parser
     [s_l_block_scalar, n, c],
    [s_l_block_collection, n, c]
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [199]
;; s-l+block-scalar(n,c) ::=
;;   s-separate(n+1,c)
;;   ( c-ns-properties(n+1,c) s-separate(n+1,c) )?
;;   ( c-l+literal(n) | c-l+folded(n) )
(def s_l_block_scalar
  (let [cache (volatile! {})]
    (name* "s_l_block_scalar"
      (fn s_l_block_scalar-fn [parser n c]
        (when DEBUG (debug-rule "s_l_block_scalar", n, c))
        (if (p/ahead-skip? parser [0x21 0x21 0x26 0x26 0x3E 0x3E 0x7C 0x7C])
          (or (get @cache [n c])
              (let [body
  (p/all parser
     [s_separate, (+ n 1), c],
    (p/rep parser
       0,
      1,
      (p/all parser
         [c_ns_properties, (+ n 1), c],
        [s_separate, (+ n 1), c]
      )),
    (p/any parser
       [c_l_literal, n],
      [c_l_folded, n]
    )
  )]
                (vswap! cache assoc [n c] body)
                body))
          false))
      nil)))
;; [200]
;; s-l+block-collection(n,c) ::=
;;   ( s-separate(n+1,c)
;;   c-ns-properties(n+1,c) )?
;;   s-l-comments
;;   ( l+block-sequence(seq-spaces(n,c))
;;   | l+block-mapping(n) )
(def s_l_block_collection
  (let [cache (volatile! {})]
    (name* "s_l_block_collection"
      (fn s_l_block_collection-fn [parser n c]
        (when DEBUG (debug-rule "s_l_block_collection", n, c))
        (or (get @cache [n c])
            (let [body
  (p/all parser
     (p/rep parser
       0,
      1,
      (p/all parser
         [s_separate, (+ n 1), c],
        (p/any parser
           (p/all parser
             [c_ns_properties, (+ n 1), c],
            s_l_comments
          ),
          (p/all parser
             c_ns_tag_property,
            s_l_comments
          ),
          (p/all parser
             c_ns_anchor_property,
            s_l_comments
          )
        )
      )),
    s_l_comments,
    (p/any parser
       [l_block_sequence, [seq_spaces, n, c]],
      [l_block_mapping, n]
    )
  )]
              (vswap! cache assoc [n c] body)
              body)))
      nil)))
;; [201]
;; seq-spaces(n,c) ::=
;;   ( c = block-out => n-1 )
;;   ( c = block-in => n )
(def seq_spaces
  (name* "seq_spaces"
    (fn seq_spaces-fn [parser n c]
      (when DEBUG (debug-rule "seq_spaces", n, c))
    (p/flip parser
     c,
    {
      "block-in" n,
      "block-out" (- n 1),
    }
  ))
    nil))

;; [202]
;; l-document-prefix ::=
;;   c-byte-order-mark? l-comment*
(def l_document_prefix
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chars-rep parser 0, 1, [0xFEFF 0xFEFF]),
    (p/rep2 parser
       0,
      nil,
      l_comment
    )
  )))]
    (name* "l_document_prefix"
      (fn l_document_prefix-fn [parser]
        (when DEBUG (debug-rule "l_document_prefix"))
        @body)
      nil)))
;; [203]
;; c-directives-end ::=
;;   '-' '-' '-'
(def c_directives_end
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "-"
    ),
    (p/chr parser
       "-"
    ),
    (p/chr parser
       "-"
    ),
    (p/chk parser
       "=",
      (p/any parser
         (p/end-of-stream parser),
        s_white,
        b_break
      )
    )
  )))]
    (name* "c_directives_end"
      (fn c_directives_end-fn [parser]
        (when DEBUG (debug-rule "c_directives_end"))
        @body)
      nil)))
;; [204]
;; c-document-end ::=
;;   '.' '.' '.'
(def c_document_end
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/chr parser
       "."
    ),
    (p/chr parser
       "."
    ),
    (p/chr parser
       "."
    )
  )))]
    (name* "c_document_end"
      (fn c_document_end-fn [parser]
        (when DEBUG (debug-rule "c_document_end"))
        @body)
      nil)))
;; [205]
;; l-document-suffix ::=
;;   c-document-end s-l-comments
(def l_document_suffix
  (let [body (delay
               (let [parser nil]
    (p/all parser
     c_document_end,
    s_l_comments
  )))]
    (name* "l_document_suffix"
      (fn l_document_suffix-fn [parser]
        (when DEBUG (debug-rule "l_document_suffix"))
        @body)
      nil)))
;; [206]
;; c-forbidden ::=
;;   <start_of_line>
;;   ( c-directives-end | c-document-end )
;;   ( b-char | s-white | <end_of_file> )
(def c_forbidden
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/start-of-line parser),
    (p/any parser
       c_directives_end,
      c_document_end
    ),
    (p/any parser
       b_char,
      s_white,
      (p/end-of-stream parser)
    )
  )))]
    (name* "c_forbidden"
      (fn c_forbidden-fn [parser]
        (when DEBUG (debug-rule "c_forbidden"))
        @body)
      nil)))
;; [207]
;; l-bare-document ::=
;;   s-l+block-node(-1,block-in)
;;   <excluding_c-forbidden_content>
(def l_bare_document
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/exclude parser
       c_forbidden
    ),
    [s_l_block_node, -1, "block-in"]
  )))]
    (name* "l_bare_document"
      (fn l_bare_document-fn [parser]
        (when DEBUG (debug-rule "l_bare_document"))
        @body)
      nil)))
;; [208]
;; l-explicit-document ::=
;;   c-directives-end
;;   ( l-bare-document
;;   | ( e-node s-l-comments ) )
(def l_explicit_document
  (let [body (delay
               (let [parser nil]
    (p/all parser
     c_directives_end,
    (p/any parser
       l_bare_document,
      (p/all parser
         e_node,
        s_l_comments
      )
    )
  )))]
    (name* "l_explicit_document"
      (fn l_explicit_document-fn [parser]
        (when DEBUG (debug-rule "l_explicit_document"))
        @body)
      nil)))
;; [209]
;; l-directive-document ::=
;;   l-directive+
;;   l-explicit-document
(def l_directive_document
  (let [body (delay
               (let [parser nil]
    (p/all parser
     (p/rep parser
       1,
      nil,
      l_directive
    ),
    l_explicit_document
  )))]
    (name* "l_directive_document"
      (fn l_directive_document-fn [parser]
        (when DEBUG (debug-rule "l_directive_document"))
        @body)
      nil)))
;; [210]
;; l-any-document ::=
;;   l-directive-document
;;   | l-explicit-document
;;   | l-bare-document
(def l_any_document
  (let [body (delay
               (let [parser nil]
    (p/any parser
     l_directive_document,
    l_explicit_document,
    l_bare_document
  )))]
    (name* "l_any_document"
      (fn l_any_document-fn [parser]
        (when DEBUG (debug-rule "l_any_document"))
        @body)
      nil)))
;; [211]
;; l-yaml-stream ::=
;;   l-document-prefix* l-any-document?
;;   ( ( l-document-suffix+ l-document-prefix*
;;   l-any-document? )
;;   | ( l-document-prefix* l-explicit-document? ) )*
(def l_yaml_stream
  (let [body (delay
               (let [parser nil]
    (p/all parser
     l_document_prefix,
    (p/rep parser
       0,
      1,
      l_any_document
    ),
    (p/rep2 parser
       0,
      nil,
      (p/any parser
         (p/all parser
           l_document_suffix,
          (p/rep parser
             0,
            nil,
            l_document_prefix
          ),
          (p/rep2 parser
             0,
            1,
            l_any_document
          )
        ),
        (p/all parser
           l_document_prefix,
          (p/rep parser
             0,
            1,
            l_explicit_document
          )
        )
      ))
  )))]
    (name* "l_yaml_stream"
      (fn l_yaml_stream-fn [parser]
        (when DEBUG (debug-rule "l_yaml_stream"))
        @body)
      nil)))

